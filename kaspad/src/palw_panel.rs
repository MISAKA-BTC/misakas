...
impl PalwPanelService {
    ...
    fn close_source_from_served_intervals_v1(
        ...
        let Some(prompt_ids) = Self::fp_prompt_for_job(backend, job) else {
        ...
        let Some(output_ids) = pooled.iter().cloned().chain(disk).find_map(|bytes| {
            palw_fp_job_material_decode_v1(&bytes)?;
            palw_fp_committed_output_ids_decode_v1(&bytes, |capture| backend.fp_committed_output_ids(capture))
        }) else {
