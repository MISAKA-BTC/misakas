...
async fn equivocate(args: EquivocateArgs) -> Result<(), String> {
    ...
    let evidence = match args.kind.as_str() {
        ...
        kind => {
            ...
            // No VLT voting snapshot exists to commit to, so both precommits carry the zero
            // snapshot commitment an honest signer supplies.
            let snapshot_commitment = Hash64::default();
            let a = accused.sign_precommit(
                net_id.as_byte_slice(),
                epoch,
                anchor,
                target_daa_score,
