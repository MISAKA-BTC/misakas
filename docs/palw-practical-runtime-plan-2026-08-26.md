...
**忠実になった**: 4層で float 参照に対し **コサイン 0.9952 / rank 相関 0.873 / top-1 7/8 / top-5 8/8**、
`FAITHFUL`。1層では **0.9992 / 0.960 / top-1 8/8**。線形アームの各サイトは
`attn_norm 1.0000 → linear_qkv 1.0000 → linear_decay 1.0000 → linear_state 0.9996 →
linear_normed 1.0000 → linear_gated 0.9999 → linear_out 0.9996`、
gated-attention アームは `attn_q 0.9998 → attn_q_rot 0.9992 → attn_values 0.9967 → attn_out 0.9978`。

**テキストが入ってテキストが出る**: `qwen36-chat` がトークナイザ（GGUF ヘッダ内の
248,320 語彙・247,587 マージ）を読み、Qwen3.6 のチャットテンプレートを組み、prefill して貪欲デコードする。
英語・日本語ともにラウンドトリップ一致。

### 忠実さを止めていた欠陥（発見順、どれもエラーを出さなかった）

**チェックポイントの読み違い 4 件** — 「モジュールが持っているもの」ではなく
「ファイルに書いてあるもの」を測って見つかった。
