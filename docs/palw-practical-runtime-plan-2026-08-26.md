...
33 GiB のクラスをヘッダ分のコストで開ける。24 GB の GGUF はディスクに置かず HTTP range で流す。

**まだ忠実ではない**: 1層で float 参照に対しコサイン 0.42、rank 相関 0.14。
サイトごとのコサインで**残る劣化を 2 段に特定済み**:

```
attn_norm 1.000 → linear_qkv 1.000 → linear_conv 1.000 → state_out 0.9999
→ normed 0.983 → gate_act 1.000 → gated 0.707 ←(1)
→ residual 0.697 → ffn_norm 0.597 → router 0.930 → routed 0.322 ←(2)
→ logits 0.572
```

排除済みの仮説:
- **解像度ではない** — 参照の行を較正済み指数で量子化して掛けるとコサイン 0.99998
- **レーン別スケールでは直らない** — 試したら 0.98 → 0.22。活性化はこの後 reduction されるので、
  レーンごとに違う指数は「足せない数の和」になる
- **`a16_rms_norm` ではない** — f64 参照に対しコサイン 1.000000

途中で見つけて直した実欠陥（どれもエラーを出さなかった）:
1. 残差加算の飽和（delta を自分の指数に置くと、揃えたストリームが桁あふれ）
2. 状態指数を縮小境界から導出（decay≈1 のヘッド 1 個で 10⁶ を取り、実測 2.6 が飽和）
3. 再帰の乗数が 2⁶² （i64 境界 2³⁰ 超過）
4. **conv のタップが転置** — GGUF は `[channel][tap]`
5. **出力ノルムが行全体** — `ssm_norm` は `[head_dim]` なのでヘッドごと
6. router の softmax 幅が shape 定数（40層で共有、最大 64× の温度誤差）
7. 埋め込みが 248,320 行に 1 スケール（外れ値のスケール）
8. `silu` に Q[K] でなくコード行を渡していた（conv と z の 2 箇所）
