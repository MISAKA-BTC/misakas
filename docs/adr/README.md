...
| 0056 D6 — "an attempt whose `artifact_root` differs is admission-rejected" | amended (design, PROPOSED 2026-09-05): differs from a root *in force* — the founding root stays written once, and a successor root named by an exam is in force beside it for a grace; `DuplicateClass` stands | [0088](0088-the-class-keeps-its-graph-and-the-exam-names-its-weights.md) D1 |
| 0087 D4 — the registrant leg is the class's registrant's; D7 — "a new version is a new class with a new market" | amended (design, PROPOSED 2026-09-05): the leg gains a second payee, the author of the weights in force; D7 is narrowed to a new *graph* — new weights on the same graph succeed inside the class by exam, and the market stays | [0088](0088-the-class-keeps-its-graph-and-the-exam-names-its-weights.md) D9, D1 |
...
| [0088](0088-the-class-keeps-its-graph-and-the-exam-names-its-weights.md) | PROPOSED 2026-09-05, design only. The class keeps its graph and its weights are versions: a successor root is proposed by a bond with rent, frozen before the questions exist, examined on items the beacon draws from a permissionless pool of pure integer programs that emit token ids, answered by court-triable claims, graded by an id-prefix comparison in the fold, and made the root in force by two integer inequalities; a chained index of accepted improvements is the benchmark the market reads; the registrant leg gains the author as a payee, an optional bounty buys the author a position from the curve. Amends 0087 D4/D7 and 0056 D6's admission clause; a consensus rule armed by activation, and no state-root bump — the rows enter the root only when non-empty, as ADR-0087's implementation does. |
