...
| 0087 D1 — the market is per class; D4 — the registrant leg is the class's registrant's; D7 — "a new version is a new class with a new market" | amended (PROPOSED 2026-09-05, revised the same day; implemented the same day on `palw-adr0088-0089-impl`): the market is keyed by a *line*, of which the class's own is the first (its id the class id, so every class with only its own line is byte-identical); the leg is the line's *owner's*, shared with an adopted contributor when the owner says so; D7 is narrowed to a new *graph* — new weights are a new *version* of a line, published by its developer, and the market stays | [0088](0088-the-class-keeps-its-graph-and-the-owner-keeps-publishing.md) D9, D8, D2 |
| 0020 §"Consequences" — "the node binary remains secp-free by default; EVM is an opt-in (`--features evm`) build" | contradicted by the tree since 2026-08-21: `kaspad/Cargo.toml:93` is `default = ["evm"]` and a non-`evm` build refuses an EVM-active network (`EvmLaneRequiresEvmBuild`); F001 WMISAKA is declared and never deployed; F002 is a call-frame intercept, not a precompile. Recorded, not reversed — the rest of 0020 stands | [0089](0089-the-fold-is-the-truth-and-the-evm-is-its-window-and-its-hand.md) §2 |
| 0087 D3 — two objects on carriers are the only moves; D8 — the surfaces; §1 — "the EVM lane is an optional, non-default feature" | amended (PROPOSED 2026-09-05; implemented the same day on `palw-adr0088-0089-impl`): the same two moves can be made from the EVM through a writer whose actions the fold applies after the block; the surfaces gain three read precompiles and a per-class MRC-20 facade; the premise is stale (the lane is a default build) | [0089](0089-the-fold-is-the-truth-and-the-evm-is-its-window-and-its-hand.md) D5–D7 |
| 0087 D1 — 100,000 positions of 10^6 units; D2 — a virtual reserve `V` opens every market by itself on the first buy; D3 — two moves; D4 — a leg on every move; 0089 D3/D5/D6 — two actions, two events | amended (PROPOSED and IMPLEMENTED 2026-09-05 on `palw-adr0088-0089-impl`): a position is whole and there are 500,000 a line; a market opens ONLY by a seed of at least 100,000 MSK that becomes the reserve, fee-free, locked for good (the curve's product never falls, so the reserve never falls under the seed); a third move `ModelSeed` / writer action 3 / facade `seed()` with a `Seeded` event; `constants()` carries the least seed where the virtual reserve was; `decimals() == 0` | [0090](0090-the-pair-is-seeded-with-real-msk-locked-for-good-and-a-position-is-whole.md) D1–D5 |
| 0087 D3 — the moves on the row are the two (three since 0090) objects' and the writer's; D4 — a leg on every move; D8 — the row's words; 0088 §10 — "a per-line service fee … a share of a claim's escrowed reward to the owner" left undecided; 0089 D2 — `market()`'s words; 0090 D2/§5 — the reserve grows by buys, and the conservation identity has two sources | amended (PROPOSED 2026-09-06 on `palw-adr0088-0089-impl`, design first at the operator's word): at a claim's `Final` FIVE PERCENT of its escrowed worker reward (ADR-0042 D10's carve — the miner's whole subsidy reward on every shipped preset) buys from the pair of the LINE the claim ran, the positions the curve gives up are RETIRED (the chain's, for good; M1 counts them), no leg and no holder, and the other 95 % is named for the miner as before; no pair (or a closed one, or an unattributed claim) means the miner is paid in full — nothing is burned for a model; the service fee is decided *no*; the row and `market()` gain `buyback_sompi` / `retired_units` at the end | [0091](0091-the-reward-buys-the-pair-and-no-holder-is-paid.md) D1–D8 |
...
| 0077, 0078, 0079 | Written 2026-09-02 on three sessions' own branches and **landed here the same day**, texts and security amendments together: [0077](0077-a-prompt-a-person-would-type-is-a-claim-the-court-can-try.md) (`70084763`…`d3a6190b`), [0078](0078-what-was-made-from-it-is-committed-the-thing-never-rides.md) (`4047e86c`, `bb14b63a`), [0079](0079-a-pure-function-needs-no-permissions-the-sandbox-is-for-the-host.md) (`9f97db97`). Only the ADR text is here; the implementation branches (`palw-adr0078-impl` and the `palw-adr0077-*` set) carry code that is theirs to land. `palw-adr0078-impl` also edits ADR-0078's §6 as it goes, in a different region from the security amendment appended at the end of that file, and merging it into this `main` was measured clean on the day this row was written. Each ADR's own §"Number hygiene" says a concurrent claimant renumbers the later writer; the numbers are now resident, so the next free number was 0080; 0080–0088 are resident now (0083 on `palw-daa-bits-priced-rows`, 0084–0087 on `palw-adr0084-served-answer`, 0088 and 0089 on `palw-adr0088-0089-impl`, their texts and their implementation together) and 0090 is resident on `palw-adr0088-0089-impl` (2026-09-05, with its implementation), as is 0091 (2026-09-06, design first); 0092 and 0093 were taken while 0091 was written, 0094 and 0095 are resident here (2026-09-07), 0096 here too (written 2026-09-10 on `feat/adr-0096-everyday-lane` and merged with its implementation on 2026-09-11), 0097 on its own branch (`feat/adr-0097-model-fit`), and 0098 here (2026-09-10, its node-side half cherry-picked from `feat/adr-0098-seat-coverage`); 0099–0101 were taken on other branches on 2026-09-10 and are not resident in this `main`, so [0102](0102-a-heartbeat-never-turns-a-bonded-block-red.md) is resident here (written 2026-09-11 on `fix/heartbeat-trap-slow-producers` with its implementation) with those three skipped rather than reused; **the next free number is 0103.** |
...
| [0084](0084-the-ids-ride-the-capture-stays-home.md) | PROPOSED 2026-09-04, implemented consensus-inert on `palw-adr0084-served-answer` (no fence; the fingerprint does not move). A model-class claim's served material is bounded by its ANSWER: a third free-prompt payload `FPA1` (the job, the prompt ids, the answer's ids, bound to the claim's `output_root` by the ADR-0078 X6 recompute), served by the resolver in place of any material over `PALW_MATERIAL_MAX_BYTES`; nothing over the cap is pushed and the block goes before the announcement; the interval arm serves the attempt lane too; the node names its retention directory over RPC and the submitter stages `<claim>.answer` beside the material. Measured on 5f: 253 MB / 748 MB materials against a 16 MiB cap voided every model-class claim, and the two public free-prompt claims were staged where the node never looked. |
| [0085](0085-the-close-is-assembled-from-what-was-served.md) | PROPOSED 2026-09-04, items 1–3 landed 2026-09-04; items 4–5 (with ADR-0086 D6's transport) under the 2026-09-05 goal. ADR-0084's U-07c: a challenger that holds no capture assembles the court's close from the interval opening's close annex — the accused's disputed tile and its rows root, the only two terms that must be the accused's — plus its own interval replay and recomputed state, checked leaf by leaf against the accused's committed hashes. No new message, nothing on chain moves. |
| [0086](0086-the-opening-carries-the-fold-not-the-leaves.md) | PROPOSED 2026-09-05, implemented consensus-inert on `palw-adr0084-served-answer` (no fence). ADR-0084 §7.2's first option: the interval opening carries the fold's digests and the Merkle frontier, never the range's leaf hashes and never the state's chunks — the seat replays and supplies its own leaves. A graph-v5 opening falls from 424 MB / 7.8 MB to ≈110 KB / ≈1.1 MB. |
| [0087](0087-a-position-is-bought-from-the-curve-and-sold-back-to-it.md) | PROPOSED 2026-09-05; IMPLEMENTED 2026-09-05 behind `Params::palw_model_market` (`None` on every preset): state, objects, fold, fence, RPC, CLI, 12 tests (§7); the site `web/misaka-options/` (misakaoptions.com) and the 2026-09-05 devnet drill (buy/sell on both lanes; the sink's output class carved out of the mempool and consensus rules on the way) landed later the same day; retire/drain under the drill and the testnet-11 activation are outstanding. Model Positions: a fixed-supply, per-class position bought from and sold back to a protocol-owned constant-product curve in MSK, never transferred between holders; 5 % of every MSK leg burned, 1 % to the class's registrant; a consensus rule armed by activation (state v21). **§0 (2026-09-08) is the canonical statement that a position is NOT a share** — no issuer, no dividend, no claim on anything, no vote, no transfer, no maturity, each clause named against the rule that makes it so — and states the one path by which a model's added value reaches a position: use → the escrowed worker reward → 0091's five percent buys the line's pair and retires the units → the curve pays more. What a holder receives is 0095's service, from a grant set the fold closes; the section ends with the vocabulary every surface must use and with the reminder that intent is not a legal verdict. |
| [0088](0088-the-class-keeps-its-graph-and-the-owner-keeps-publishing.md) | PROPOSED 2026-09-05, **revised the same day** (the first draft's exam withdrawn by the operator: not distributed training, a distributed market); **IMPLEMENTED 2026-09-05 behind `Params::palw_model_lines` (`None` on every preset)** on `palw-adr0088-0089-impl`: state, the ten objects, fold, admission, court roots, fence, RPC, CLI, 15 tests (§8); the free-prompt job v6, the SDK flags, the explorer pages and the registry half of the devnet drill are outstanding (the market drill of 2026-09-05 read the founding line). The class keeps its graph; a *line* — a class, an owner bond, a name — keeps its owner, and the owner's developer publishes *versions* as signed objects, as often as it likes: every version recorded (root, parent, declared runtime/dataset/config hashes, declared evaluations from anyone, chain-counted usage), previews before promotion, a grace for producers, rollback by re-publish, proposals from any bond adopted with a contributor share of the fee leg, owner/developer/maintainer roles and ownership transfer (positions never move), competition between lines on one class; the market keyed by line (the founding line's key is the class id); fence `palw_model_lines`, no state-root bump. **§2.1 (2026-09-08)** says what a line's position is not: the owner is a publisher, not an issuer, and takes no proceeds from a buy (the net leg is the pair's); the seed is a locked floor, not capital, with no position and no subscribers; what is bought is the model's use plus 0095's declared service; competition between lines is between products; the registry's rows are provenance, never a prospectus. 0087 §0 is the canonical version. |
| [0089](0089-the-fold-is-the-truth-and-the-evm-is-its-window-and-its-hand.md) | PROPOSED 2026-09-05; **IMPLEMENTED 2026-09-05 behind `Params::palw_model_evm` (`None` on every preset)** on `palw-adr0088-0089-impl`: fold (actions, settlements, the EVM holder namespace), executor (three read doors, the writer, the facade family, `MarketSettle` system ops in a system receipt), processor (the action list into `fold(B)`, the settlement ops and sink outputs at `C`, the template), `eth_call` at the head, Solidity interfaces, 14 tests (§9); **devnet drill PASSED 12/12 on 2026-09-05** (`scripts/misaka-palw-model-market-devnet-e2e.sh`: carrier buy, EVM buy/refused/sell settled across `B`/`C`, six nodes agreeing, a partition's minority settlement re-included after the heal; three private-devnet flags added); the site `web/misaka-options/` is live at misakaoptions.com (wRPC reads; the EVM window needs a node serving `--evm-rpc-listen`); a historical `eth_call` view, the `CLASSICAL-ECC` label and the wallet label are outstanding. The fold is the truth; the EVM is its window and its hand: three read precompiles (`ModelRegistry` 0x…F010, `ModelAMM` 0x…F011, `ModelPosition` 0x…F012) return the fold's rows at the EVM block's selected parent; `ModelWriter` 0x…F013 is CoreWriter's shape with two actions (buy, sell) applied by the fold after the block and settled by system ops in the selected child (escrow → sink output, credit, refund); every class has an MRC-20 facade — ERC-20's read half, the curve in place of its transfer half, `supportsInterface(ERC-20) == false`; holders are signing accounts only, so no contract can rebuild a transfer; two holder namespaces that never cross; fence `palw_model_evm` requiring `palw_model_market` and an EVM-active preset. Hyperliquid's HyperEVM is the reference, cited with dates. |
| [0090](0090-the-pair-is-seeded-with-real-msk-locked-for-good-and-a-position-is-whole.md) | PROPOSED 2026-09-05; **IMPLEMENTED 2026-09-05 on `palw-adr0088-0089-impl`** (§8) under the same three dormant fences. The pair is seeded with real MSK, locked for good, and a position is whole: a line's market opens only by a seed of at least 100,000 MSK that becomes the reserve with no fee and no position for the seeder; 500,000 whole positions in the curve; `reserve × units` never falls, so the reserve never falls under the seed; buys raise the price; the seed may precede the class's approval (Active) but no buy may; a third move on both lanes (`ModelSeed`, writer action 3, facade `seed()`, `Seeded`). Drilled on devnet the same day (ADR-0089 §9's drill, extended). Arming on testnet-11 is the outstanding release. |
| [0091](0091-the-reward-buys-the-pair-and-no-holder-is-paid.md) | PROPOSED 2026-09-06 (design first, at the operator's word; the implementation follows on `palw-adr0088-0089-impl` under the same three dormant fences). The reward buys the pair, and no holder is paid: five percent of the worker reward a block earns with a model buys positions from that model's pair at the claim's `Final`, the chain retires what it buys, the other ninety-five percent is paid to the miner as before, and nothing is ever distributed to a holder — the reward's only effect on a holder is the price the curve will pay them. Replaces the operator's earlier reading (a share of the reward distributed to holders), withdrawn as the shape of a security. |
| [0094](0094-a-seed-is-paid-in-as-many-transactions-as-it-takes.md) | PROPOSED and **IMPLEMENTED 2026-09-07** under the same `palw_model_market` fence. A seed is paid in as many transactions as it takes: a hundred thousand MSK does not fit in one post-quantum carrier — fifteen ML-DSA-87 inputs is the 480,000-mass cap's most, measured on the live chain — so ADR-0090's "one seed a line" becomes an accumulation. Every payment locks in the line's sink on arrival; the market opens on the payment that carries the total across the floor, with the WHOLE collected total as the reserve; under the floor there is a row with no positions, no price and no moves. The row names the first payer. The CLI stops asking for one utxo and spends up to fifteen. |
| [0095](0095-a-position-is-a-membership-not-an-income.md) | PROPOSED 2026-09-07. A position is a membership, not an income: it buys no share and no vote, it buys what its LINE declares — a new version's artifact ahead of the promotion, the private beta, the front of the inference queue, experimental modes, the developer's own room. A line's OWNER declares up to eight tiers by held units over a CLOSED grant set (nothing that pays is in it); the tier is a pure function of the carrier + EVM balance at a named height; a holder proves it by signing a challenge, with no transaction. The chain proves the holding, publishes the promise and records both the preview and the promotion height so a broken lead is arithmetic — it does not serve, and does not pretend to. §4.4 is the teeth: the fold REFUSES a promotion before `published + lead`, and refuses a straight-to-current publish while a lead stands, so the window is arithmetic rather than a promise; §4.6 lapses a declaration that outlives its cadence or expiry, with no object submitted; §4.7 makes a WEAKENING wait out a notice while the old promise governs. Its OWN fence `palw_model_benefits` (the first draft said otherwise and was wrong: it writes rows that enter the state root), scheduled on testnet-11 at DAA 2,400, `None` everywhere else. |
| [0096](0096-the-app-you-already-use-is-the-entrance-and-the-shape-of-the-answer-is-committed.md) | PROPOSED 2026-09-10 on `feat/adr-0096-everyday-lane` (both repositories). The app you already use is the entrance, and the shape of the answer is committed: **Part A** (host-side) — one OpenAI surface spelled once and served by the Studio's `/v1` and the gateway (content parts, `tools`/`tool` turns as the model's own Hermes-style text under new template ids, `response_format`, refuse-by-name for what neither serves); sampling refused by the gateway as ADR-0082 D11 says and mapped WITH NOTICE by the person's own app (`misaka.sampling`); a long thread is a chain of jobs (trim → summary job → continue legs, all listed in `misaka.jobs[]`), the row never moves (ADR-0092 D4). **Part B** (a ruleset move behind `Params::palw_fp_decode_constraint`, `None` everywhere, arming REFUSED at assembly until the build carries all three halves, ADR-0082 D11's shape) — the job gains `constraint_id` (v6), the committed token is the argmax over the lanes a pinned byte-level automaton admits (reduces to D11's rule at `0`), the refutation keeps its two-disclosure form and gains a state plus a one-disclosure "not admitted" arm, the answer is cut at the first committed EOG id by a versioned rule (amends ADR-0078 D6 past the fence only), and a `json` kind (RFC 8785) is row 8 of ADR-0078 D8; until the fence arms, `response_format` is served ADVISORY and the answer says so (`misaka.format.enforcement`), with `require_committed_format` refused by name before the inference. **Part C** — one `components.json` manifest published by both releases and checked across them (the Studio's `misaka` engine drove `misaka-palw-serve`, deleted from this tree on 2026-09-02 by `2f688bc1`, and nothing knew); every settings panel shows the EFFECTIVE value read from the running object and the rebuild predicate is a constructor fingerprint; conversations become the runtime's (file + API + import from OpenAI's export); a model request gets a door (issue form + `docs/model-requests.md` + a button) that is not chain state. |
| [0098](0098-the-panels-coverage-is-a-number-and-a-seat-that-found-a-lie-files-nothing-else.md) | PROPOSED 2026-09-10 on `feat/adr-0098-seat-coverage` (from `feat/adr-0097-model-fit`); its node-side Decisions 2–3 and the Decision 1 report were cherry-picked onto `main` for the 2026-09-10 fleet rebuild; **Decisions 1–3 IMPLEMENTED the same day**, node-side and consensus-inert (no object, fence or fingerprint). The panel's coverage is a number, and a seat that found a lie files nothing else. **Decision 1** answers ADR-0081 D8 / U-03 for the LIVE draw (ADR-0077 D8): measured with `palw_fp_interval_draw_v1` itself and in closed form (`palw_seat_coverage_v1`, generator `palw-seat-coverage`): a one-token lie on the 300-token claim testnet-11 licensed is caught 6.51 % of the time with five replaying seats and 3.96 % with three — 35 forged claims before 90 % odds of one catch; 90 % needs 111 of 299 intervals per seat; a cache-altering lie is caught by any later draw through the state root, so the selection lie at the end is the floor. **Found on the way (§1.3):** a seat that found a fault fell through the same round to the capture arm, which could file `Valid`, or to the half-window tail, which filed `Unavailable` — charged `claim.reserved` on a ruleset without ADR-0065 D4 (the shipped devnet) when the rest of the panel licensed the forged claim; a checkpoint state root that did not recompute, and a capture leaf that did not recompute, were dropped unrecorded. **Decisions 2–3** restore "the seat files nothing" (a fault ledger consulted at three points; every proven fault recorded; silence is never charged). **Decision 4**: the expected cost of a lie is the number × `claim.reserved`. **Decision 5** names what a network of shard-holding seats needs: a stratified panel (coverage independent of the shard count; licensing fits one standard transaction up to eight shards at a quorum of three) and a court opened at a NAMED leaf — because today every conviction needs a party that can re-execute the whole job. **Decision 6** leaves the pending-openings fall-through to the operator. |
| [0102](0102-a-heartbeat-never-turns-a-bonded-block-red.md) | PROPOSED 2026-09-11 on `fix/heartbeat-trap-slow-producers`. The slow-producer trap testnet-11 fell into on 2026-09-10 (11:41Z–13:20Z): once a heartbeat is the selected parent, a bonded draw that takes ~17 minutes lands eight heartbeats behind the tip and, at `ghostdag_k = 1`, is merged RED — its 2²⁰ in no block's blue work, DNS `workDepth` stuck near 10 against 100, the bridge paused while any heartbeat miner runs. **Decision 1 IMPLEMENTED behind `Params::palw_heartbeat_transparent` (`None` on every preset; no pin moves)**: past the fence GHOSTDAG colors a non-heartbeat candidate against non-heartbeat blues alone (heartbeats still yield to bonded blocks), keyed on the candidate's own DAA score, and the exemption stops at the merge-depth window so no blue it creates is deeper than an admissible red. Measured in the pipeline: dormant, each merging heartbeat adds 2 and the episode's work is 36 < 100; armed, each adds 1,048,577. **Decision 2 IMPLEMENTED node-side**: the heartbeat miner stands aside while an attempt block waits in the virtual's mergeset (`heartbeat_yield_hint_v1`), one nominal hour per episode, so the next draw takes the chain back on today's rule. Rejected, with reasons: a longer recovery cadence, a consensus slot hold (lets a stuck producer hold the clock), chainwork-including-self (every blue-work ordering; circular fence). Arming on testnet-11 is a flag day (§7). |
