...
| [0097](0097-a-models-fit-is-a-lookup-and-the-entrance-says-its-limits-before-the-first-token.md) | PROPOSED 2026-09-10 on `feat/adr-0097-model-fit` (measured at `70d56519`, rebased onto `feat/adr-0096-everyday-lane`'s `421cc92e` with the numbers unchanged); **IMPLEMENTED the same day, consensus-inert** (no fence, no field, no object; the fingerprint does not move). A model's fit is a lookup, and the entrance says its limits before the first token: **Decision 1** — `palw_model_fit_v1` reports every wall a class meets on a ruleset (geometry ceiling, ladder, the four close ceilings, court window, state chunks, PublicDa payload) as `need / have / verdict` from the SAME predicates admission and the court run, and `misaka-palw-base0 --bin palw-model-fit` is the generator (ADR-0092 §5: no ADR writes a number it does not print). Measured on the RC: the dense graph-v5 row's fit is 512 and the wall at 513 is the court window alone (the ladder would take it to 651); the hybrid row is held at 204 by the ladder alone and the devnet cannot carry it at all (one carrier); a Kimi K3 stand-in (`stand_ins::KIMI_K3_AS_HYBRID_V1`, every substitution named) fits at **ten positions**, is refused by the ladder alone at 512 and by four walls at 131,072, and has no profile at its card's 1M — the geometry ceiling `PALW_STEP_MAX_ENUMERATION` refuses a 2M context for every model deeper than eight layers and a 1M context past sixteen. **Decision 2** — `GET /v1/models` and `/health` carry `misaka.limits` (the window, the answer ceiling, the prompt bytes, the tokenizer id, and per feature the word each fence decides), the two pre-chain refusals carry `misaka.refusal.code` beside the unchanged sentence, and the `max_tokens` clamp is reported in `misaka.decode`. **Decisions 3–5** — the geometry ceiling is a wall with a name and is not moved (a ruleset move under ADR-0092 D4); a stand-in is a verdict, never a row; a K3-class network is a different mint whose first requirement is a seat that holds a shard and not the model, named wall by wall with the measurement each needs and taken by none. Appendix A reads the operator's sixty-item checklist against the tree. |
