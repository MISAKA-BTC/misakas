...
| [0096](0096-the-everyday-surface-is-the-gateways-and-the-constrained-token-is-the-chains.md) | PROPOSED 2026-09-10; **Part B's fence, its wire and Decision 8's rule IMPLEMENTED 2026-09-10** on `claude/adr-0096-partb-fence-uwo64p` (§9 is the running record; Part A, the constraint crate and the engine are outstanding). The everyday surface is the gateway's, and the constrained token is the chain's. Splits what an OpenAI-shaped client asks for into two halves along one line — the chain hashes RENDERED PROMPT BYTES, so anything expressible as "these bytes instead of those" is outside consensus. **Part A, consensus-inert:** content parts (non-text refused by name, never dropped), tools rendered in the class's own chat-template format with the gateway never executing one, `/v1/models` answering only the class this gateway is bound to, refusals that name their rule, and `response_format` accepted as ADVISORY below the fence with a `misaka.format` object stating `enforced`/`valid` — the asymmetry with `temperature` argued from whether the dormant fence changes what the TRANSITION does with the result. **Part B, a consensus rule behind its own dormant fence `palw_fp_decode_constraint`** (`None` on every preset, **arming REFUSED by `validate_palw_v2` on this build**, ADR-0082's own refusal shape): the committed token is the seeded argmax over ADMITTED lanes only, admission a predicate on `(automaton state, lane)` and nothing else — so ADR-0049 D E's two-disclosure refutation is unchanged and gains one arm (a committed lane that is not admitted convicts on one disclosure). Separate from `palw_fp_decode_rules` deliberately: a constraint is well-defined at `T_q = 0`, and constrained greedy is what `response_format` means. Also: `json` as kind 28, a runtime's effective-settings object and fingerprint, and one components manifest with one resolution order. §1 is measured against `main` at `7f4dded` and corrects two claims from the previous pass — 256 was the `max_decode_default`, never a wall (`--max-decode-cap` 1024, hard 4,096), and `0001f34c` chunks the retained-trace manifest rather than lifting any decode ceiling. |
