...
| 0077, 0078, 0079 | Written 2026-09-02 on three sessions' own branches and **landed here the same day**, texts and security amendments together: [0077](0077-a-prompt-a-person-would-type-is-a-claim-the-court-can-try.md) (`70084763`…`d3a6190b`), [0078](0078-what-was-made-from-it-is-committed-the-thing-never-rides.md) (`4047e86c`, `bb14b63a`), [0079](0079-a-pure-function-needs-no-permissions-the-sandbox-is-for-the-host.md) (`9f97db97`). Only the ADR text is here; the implementation branches (`palw-adr0078-impl` and the `palw-adr0077-*` set) carry code that is theirs to land, and `palw-adr0078-impl` also edits ADR-0078's §6 as it goes — the merge that brings it will reconcile that against the security amendment appended below the same file. Each ADR's own §"Number hygiene" says a concurrent claimant renumbers the later writer; the numbers are now resident, so **the next free number is 0080.** |
