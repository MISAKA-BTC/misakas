...
| [0085](0085-the-close-is-assembled-from-what-was-served.md) | PROPOSED 2026-09-04, design only (implementation not started). ADR-0084's U-07c: a challenger that holds no capture assembles the court's close from the interval opening's close annex — the accused's disputed tile and its rows root, the only two terms that must be the accused's — plus its own interval replay and recomputed state, checked leaf by leaf against the accused's committed hashes. No new message, nothing on chain moves. |
...
| [0088](0088-a-carriers-funding-is-the-callers-to-name.md) | ACCEPTED 2026-09-05, implemented consensus-inert (no object, rule, parameter or fingerprint moves). Two host-side tools were asking the chain's own numbers a question they were not the answer to: `misaka wallet send` could not fund a second carrier inside one block interval (it selects from the UTXO set, so it rebuilt the same transaction and the node answered "already in the mempool"), and the gateway's day-budget read the exposure ROOM, charging every open claim twice — once as the chain's reservation, again in its own counter — so a bond sized for four concurrent claims stopped at two. Adds `--funding-outpoint` and `--spend-unconfirmed` to the wallet, and budgets the gateway's day against the bond's ceiling. |
