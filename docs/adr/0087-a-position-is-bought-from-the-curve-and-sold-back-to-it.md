...
* **Trading:** 6 % of each leg leaves the trade (5 % burned, 1 % to the registrant), so a round
  ...
  (Corrected twice, and the second correction is the one worth reading. The design draft read
  `12,846 out`, `0.0742`, `3,014 from the reserve` and `2,833 paid` — arithmetic this curve cannot
  produce: a reserve holding 1,880 pays at most 1,880, and 3,014 would have been 1,134 MSK nobody
  paid in, against M2 and against ADR-0059's carve-not-mint rule. The first correction, made with
  the implementation, fixed the positions and both MSK columns and left the second row's *price*
  at `0.0576`, a number belonging to neither reading. Reported as
  [#98](https://github.com/MISAKA-BTC/misakas/issues/98), which reproduced row 1 exactly — that is
  what establishes the curve is being read as written — and then showed rows 2 and 3 could not
  follow from it. `(1,880 + 1,000) / 34,722.22 = 0.082944`.

  Why the digit survived a correction made *with* the implementation is the part worth fixing:
  `the_adr_table_is_the_curves_arithmetic` pinned every column of this table — both buys'
  positions, all three MSK legs, the reserve after each move, the round trip — except the last
  one, where it asserted only that buying raises the price and selling lowers it. A golden table
  that checks six columns and orders the seventh will hold a wrong number in the seventh
  indefinitely. It now pins both prices exactly, at 3,763,599 and 8,294,399 sompi a position.)
  ...
  the reserve and not at `reserve + V`. A partial sell is paid on the curve's slope at that point,
  which is the only place slippage lives.
