...
2400 were scheduled. The printed fingerprint (`060e3597…`) moves; the identity does not, so a build
...
* `fork_id_v1::every_scheduled_palw_fence_moves_the_fingerprint_and_the_schedule_and_never_the_identity`
  ...
  one identity. It pins `palw_model_benefits` as the one fence whose height the fingerprint does not
  carry (known since `0448d955`; writing it moves testnet-11's pin, a release's decision), so the next
  fence forgotten the same way fails by name.
