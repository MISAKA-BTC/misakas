...
Verified against current `main` / Testnet-11 Relaunch 5f on 2026-09-13. The precommit section describes the build that schedules Testnet-11's DAA 7,001 flag day (ADR-0128).
...
From the height a network schedules ADR-0128's BFT gate (Testnet-11: **DAA 7,001**), an epoch's anchor is DNS-final only when validators holding more than two thirds of the counted bonded stake have both **attested** to it and **precommitted** to it, and the DNS stake reorg gate then refuses any chain that abandons that anchor until it goes stale. Voting power is the bond amount.
