...
it yet**. testnet-12 shipped as commit `0e8ec984e` with no tag. The versioning and freeze rules in
...
1. Everything is green on the commit: `bash scripts/ci-gates.sh` locally and the `Tests` workflow
   in CI.
2. Record the identity the release will print. Build `kaspad`, start it once, and copy the
   `Consensus params fingerprint` and `Consensus fence schedule` lines into the release notes.
3. Make a **signed, annotated tag** (`git tag -s`, or SSH signing with `gpg.format ssh`), and
   push it:
...
4. Publish a GitHub release for the tag, with **pre-release** ticked for an RC. Publishing starts
   `deploy.yaml`:
   ...
   | `sign` | `SHA256SUMS`, `SHA256SUMS.sigstore.json`, `misakas-<tag>.spdx.json`, and a provenance attestation for every asset |

   The source archives come from GitHub's own release page. `sign` needs every build leg. If one
   platform fails, nothing is signed. Fix the leg and re-run the failed jobs. The `sign` job is
   safe to re-run: it replaces its own outputs.

5. Update [Release status](../README.md#release-status) and
   [mainnet-readiness.md](mainnet-readiness.md) in the same PR as the release notes.

The network the manifests name is `RELEASE_NETWORK` at the top of `deploy.yaml`. Change it when a
release targets another network.
...
- **The ARM64 Linux leg has not run yet.** It is a native glibc build on `ubuntu-24.04-arm`. The
  first release with it is its first test.
