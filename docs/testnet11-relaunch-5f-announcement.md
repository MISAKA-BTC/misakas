...
> **Known state (2026-09-04 05:50Z):** no attempt-lane block was produced between DAA 226 and DAA 1150. Cause measured and fixed (card §10b–§10j; ADR-0083): with five heartbeat emitters on the 2-minute cadence the difficulty window read the chain as ~3× too fast and tightened `bits` by ×3,100; the fix — the window counts only rows priced by `bits` — was scheduled as a fence at DAA 1150 and fired at 05:44:54Z: the first fenced block carries `bits 0x207fffff` (Layer-0 p = 0.5). Every seat runs the flag-day build (`cef2ecdb`, fingerprint `71b35c25…`, genesis unchanged); a node on the previous build (`14065c93…`) parts from the chain at 1150 and must rebuild from tag `testnet-11-relaunch-5f-adr0083-h1150` or later. Everything above that names a claim or a block is on the chain as stated.
