...
| C-05 | **reproduces, and understates it**: the engine performs 36 steps a layer and the profile declared 18. K was never rotated; the cache roles sat on the raw projections; a leftover slot-indexed patch silently rewrote two unrelated nodes | partially fixed — the profile is generated from `BASE0_LAYER_IR` (`aace6691`). The engine is still hand-written and the IR transcribed from it |
