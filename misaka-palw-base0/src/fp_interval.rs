...
pub fn base0_open_fp_interval_chunkless_v1(
    ...
    base0_open_fp_interval_v1(material, index, prompt_token_ids, family_checkpoint_interval)
}
...
pub fn base0_fp_interval_tiles_from_fold_capped_v1<K: Base0FpIntervalKernelsV1>(
    ...
    let (operands, replay_first_call) = match geometry.anchor_covered_call(index).filter(|_| anchored) {
        None => (None, 0),
        Some(covered) => (Some(base0_checkpoint_operands_v1(binding, &material.checkpoint_chunks, &[], covered)?), first_call),
    };
    let start = match (&operands, geometry.anchor_covered_call(index)) {
        (None, _) => Base0FpIntervalStartV1::Genesis { prompt_tokens: &prompt_usize },
        (Some(anchor), Some(covered)) => {
...
mod tests {
    ...
    fn the_v4_wire_form_is_frozen_and_the_old_decoders_refuse_it() {
        ...
        let bytes = base0_open_fp_interval_v1(&material, 0, &ids, PALW_INTEGER_KV_CHECKPOINT_INTERVAL_V1).expect("opens");
    ...
    fn a_tampered_opening_is_refused_by_name() {
        ...
        assert_eq!(
            base0_verify_fp_interval_opening_v1(&opening, claim, index, &ids, leaves, interval, &FloorKernels(&artifact)),
    ...
    fn a_tiled_class_refuses_an_opening_that_carries_the_history() {
        ...
        let index = 1;
        let served =
            base0_open_fp_interval_v1(&material, index, &ids, PALW_INTEGER_KV_CHECKPOINT_INTERVAL_V1).expect("the fold form opens");
    ...
    fn a_close_from_served_intervals_is_the_close_from_the_capture_and_the_annex_changes_no_verdict() {
        ...
        for index in 0..geometry.interval_count {
            let plain = base0_open_fp_interval_v1(&material, index, &ids, interval).expect("the interval opens");
