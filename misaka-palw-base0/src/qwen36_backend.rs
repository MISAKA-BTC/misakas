...
impl PalwExecutionBackendV1 for Qwen36Backend {
    ...
    fn open_fp_interval(&self, capture: &[u8], index: u32, prompt_token_ids: &[u32]) -> Result<Vec<u8>, String> {
        ...
        // Decision 7): the span's leaf hashes are re-derived by replaying from the checkpoint the
        // interval is anchored at — this family's own kernels, the loop a seat runs to check.
