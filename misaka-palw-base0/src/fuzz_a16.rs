...
impl FuzzRng {
    ...
    ///
    /// An `Iterator` is something a caller may collect, take, zip or run to exhaustion; this is an
    ...
    ///
    /// Named `next_u64` after `rand_core`'s, which is the spelling this tree already uses — the
    /// earlier `next` here needed `#[allow(clippy::should_implement_trait)]` to survive, and the
    /// justification written beside that allow was wrong: it claimed `next` was what every RNG
    /// calls this step, in a file that spelled it both ways twice over.
