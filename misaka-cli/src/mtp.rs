...
/// nullifier.
/// C5 (`mtp palw-leaves`) — **not available on this build.**
///
/// The indexer this command wrapped read algo-4 leaf-chunk registrations and Receipt-DA
/// Object-v2 files: a scheme ConsensusV2 replaced. Its consensus types (`palw_da`,
/// `receipt_v3`, the `palw_*` RpcHeader fields) no longer exist, so the old reader cannot be
/// compiled against this node — and a reader that silently found nothing would report an
/// honest replica's work as absent. C5 needs an indexer written against the claim/receipt/court
/// lifecycle. Until then this refuses rather than emitting an empty file.
pub async fn palw_leaves(_ctx: &Ctx, _da_dir: &str, _low_hash: Option<&str>, _max_blocks: usize, _out: Option<&str>) -> CliResult {
    Err(CliError::generic(
        "mtp palw-leaves: the algo-4 leaf/Receipt-DA indexer does not apply to ConsensusV2 — C5 needs \
         an indexer over the claim/receipt/court lifecycle. Refusing rather than reporting an empty \
         result as 'no replica work'."
            .to_string(),
    ))
}
