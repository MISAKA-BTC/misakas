...
pub struct RpcRawHeader {
    ...
    /// ADR-0042 Unit C step 5: the PALW V2 chain-state root. Pre-PoW and part of the header-hash
    /// preimage on V2-lineage algo ids, so it must survive the mining round-trip.
...
impl Serializer for RpcHeader {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        ...
        // ADR-0042 Unit C (serializer v6): the PALW V2 state root.
        store!(Hash64, &self.palw_state_root, writer)?;
...
impl Deserializer for RpcHeader {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        ...
        // ADR-0042 Unit C: added in serializer v6; older peers ⇒ the zero root, which is exactly
        // what the preimage gate reads as "this header keeps no V2 state".
        let palw_state_root = if serializer_version >= 6 { load!(Hash64, reader)? } else { Default::default() };
...
impl TryFrom<RpcRawHeader> for Header {
    ...
    fn try_from(header: RpcRawHeader) -> Result<Self, Self::Error> {
        ...
        .with_palw_state_root(header.palw_state_root)
