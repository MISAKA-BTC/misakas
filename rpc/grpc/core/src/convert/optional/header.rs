...
try_from!(item: &protowire::RpcOptionalHeader, kaspa_rpc_core::RpcOptionalHeader, {
    Self {
        ...
        // kaspa-pq (ADR-0004 / design §12): utxo_commitment is 64-byte Hash64.
        utxo_commitment: item.utxo_commitment.as_ref().map(|x| kaspa_consensus_core::Hash64::from_str(x)).transpose()?,
