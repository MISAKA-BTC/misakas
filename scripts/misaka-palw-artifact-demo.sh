...
PY
  ...
  #
  # **TWO SPELLINGS OF THE SAME SEED, and the drill still needs both — but fewer than it did.**
  # `misaka key`, kaspad's `--palw-producer-key` AND `misaka-palw-fp-rail --bond-key-seed` now all
  # read the same 64-character HEX TEXT: the rail's private raw-bytes reader was replaced by
  # `kaspa_pq_validator_core::load_validator_seed`, the shared reader that also enforces audit
  # M-02's 0600/regular-file guard. So `.seed` is the format for every tool but ONE.
  #
  # The exception is the gateway's `--derive-seed`, which still calls
  # `misaka_palw_gateway::derive::read_seed` — the last copy of the private reader, and it takes
  # exactly 32 RAW bytes. That call, and only that call, gets `.rawseed`. Both files are written
  # from the same digest, so `.rawseed` is an encoding and not a second secret to keep in step;
  # when the gateway adopts the shared reader too, `.rawseed` and this note go away together.
