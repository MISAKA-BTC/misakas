        ...
        for i, line in enumerate(lines):
            ...
            for k in range(i, -1, -1):
                if re.match(r"^  [A-Za-z0-9_-]+:\s*$", lines[k]):
                    job_start = k
                    break
            reads_pin = any(READ_MARK in lines[k] for k in range(job_start, i))
            if got is None:
                bad.append(
                ...
                why = "OK" if ok else ("MISMATCH" if not expr else f"UNSET -- no step in this job reads {READ_MARK}")
                print(f"  {rel}:{j + 1}: toolchain: {value}  {why}")
                if not ok:
                    if expr:
                        bad.append(
                            f"{rel}:{j + 1}: passes ${{{{ {PIN_EXPR} }}}} but no earlier step in this job "
                            f"reads {READ_MARK}; the input expands to empty and the action installs `stable`"
                        )
                    else:
...
def check_parity():
    ...
    for name, cmd in pairs:
        if name in NOT_IN_CI:
            ...
            continue
        needle = cmd[len("bash ") :] if cmd.startswith("bash ") else cmd
        if needle in ci_src:
