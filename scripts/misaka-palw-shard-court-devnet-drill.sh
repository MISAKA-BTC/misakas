...
# LEAF (unset; ADR-0111's two paths, HELD=1 only: `fast` — node-0 serves its claims' answer
...
if [ "$LEAF" = "slow" ]; then
  log "2/5 waiting for a seat to demand the named leaf's evidence on chain (ADR-0111 Decision 3)"
  accused="$(wait_log "demanding it on chain \\(ADR-0111 Decision 3\\)" "a seat's demand")"
elif [ "$LEAF" = "fast" ]; then
  # The fast path's own line, not any accusation: a capture-route accusation here would mean a seat
  # held the capture after all, and the run would prove nothing about ADR-0111.
  log "2/5 waiting for a seat to accuse on the executor's own evidence (ADR-0111 fast path)"
...
case "$LEAF" in
  fast) log "PASS — a seat that held no capture convicted a corrupted free-prompt claim on the executor's own evidence (ADR-0111 fast path)" ;;
  slow) log "PASS — a seat that held no capture demanded the leaf on chain, and the executor's answer convicted it (ADR-0111 slow path)" ;;
