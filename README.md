...
> Status: **`testnet-21` is the public testnet.** It inherits the ADR-0048 staging-mainnet PALW
...
> DNS seed domains advertise testnet-21 peers on P2P port `26531`.
> `testnet-20` and `testnet-200` are **deprecated**: kept compilable for operators still holding
> their ledgers, but they have no seeders and their replay is unsafe under current rules (see the
> migration notes below). `testnet-10` is retired from public operation and remains only as a
...
> parameter sets both use **two-dimensional** DNS-finality confirmation. testnet-21 keeps the
...
**→ [docs/testnet-21-migration.md](docs/testnet-21-migration.md)** is the operator cutover note for
the current public net: why testnet-20 was replaced (the ADR-0045 D3-b leaf re-genesis), the new
defaults and ports, and the fact that no prior testnet state can be carried across. (The earlier
notes are kept for history: [docs/testnet-20-migration.md](docs/testnet-20-migration.md) for
200→20, [docs/testnet-200-migration.md](docs/testnet-200-migration.md) for 10→200.)
...
**`testnet-21`** — the public PALW testnet. Peers are found automatically via the public DNS seeders:

```bash
git clone https://github.com/MISAKA-BTC/misakas
cd misakas
cargo build --release -p kaspad -p misaka-cli --bin misaka -p kaspa-pq-miner
./target/release/misaka join -- --utxoindex --rpclisten-borsh=default   # --network defaults to testnet-21
./target/release/misaka node doctor                                     # ports, sync, versions, RPC surface
```

If DNS is unavailable, dial a public peer explicitly (testnet-21 listens on `26531`):

```bash
./target/release/kaspad --testnet --netsuffix=21 \
  --addpeer=95.111.236.186:26531 --utxoindex
```
...
are the only GPU backends the crate exposes. On testnet-21, PALW and algo-4 acceptance are active;
...
Start a misakas testnet node (network id `testnet-21`; PQ, DNS finality, PALW, algo-4 and the Compute Set registry are all active from genesis):

```bash
cargo run --release --bin kaspad -- --testnet --netsuffix=21 --utxoindex --rpclisten-borsh=default
```
...
- To **join the public testnet**, the node discovers peers via the misakas DNS seeders
  ...
  resolving testnet-21. **testnet-21's P2P port is `26531`** (testnet-200 used `26511`; each public
  ...
  node: `--addpeer=95.111.236.186:26531` — the address the seeders themselves advertise — (or
...
- **Connecting a wallet / RPC client — pick the right port.** Default **testnet-21** ports:
  ...
  **wRPC JSON** `28210` (WebSocket — enable with `--rpclisten-json=default`). Mainnet uses
  `26110/27110/28110`; devnet `26610/27610/28610`. (Only the P2P port differs between testnet-21
  ...
  `server 127.0.0.1:27210` → `connect`. (P2P is a separate, non-RPC port: `26531`.)
- **Headless balance (no interactive wallet).** For scripting / monitoring, query a balance in one
  shot over wRPC:
  `kaspa-pq-validator balance --node-rpc 127.0.0.1:27210 --address misakatest:q… [--address …] [--network testnet-21]`.
...
```bash
cargo run --release --bin kaspa-pq-miner -- --node-grpc 127.0.0.1:26210 --network-id testnet-21 \
...
# 3. stake a bond. testnet-21's entry floor is 10 MSK = 1e9 sompi (testnet-200 kept the
...
kaspa-pq-validator run --node-rpc 127.0.0.1:27210 --validator-key val.seed \
  --stake-bond <txid:index> --signed-epoch-db val.testnet-21.state --network testnet-21 --attest-poll-secs 3
```

> Note: the funding/`run`/`bond` subcommands want the **full** network id (`testnet-21`); `keygen`'s `--network` takes the short form (`testnet`). Use a **fresh** `--signed-epoch-db` per network — reusing one across networks trips the anti-equivocation guard on overlapping epoch numbers.
