...
pub trait ReachabilityService {
    ...
    fn is_chain_ancestor_of(&self, this: BlockHash, queried: BlockHash) -> bool {
    ...
    fn try_is_chain_ancestor_of(&self, this: BlockHash, queried: BlockHash) -> Result<bool>;

    /// Result version of [`Self::is_dag_ancestor_of`] (avoids unwrapping internally)
    fn try_is_dag_ancestor_of(&self, this: BlockHash, queried: BlockHash) -> Result<bool>;
    ...
    fn is_dag_ancestor_of(&self, this: BlockHash, queried: BlockHash) -> bool {
    ...
    fn is_dag_ancestor_of_any(&self, this: BlockHash, queried: &mut impl Iterator<Item = BlockHash>) -> bool;

    /// Checks if any of the blocks in `list` is DAG ancestor of `queried`. See [`Self::is_dag_ancestor_of`] as well.
    fn is_any_dag_ancestor(&self, list: &mut impl Iterator<Item = BlockHash>, queried: BlockHash) -> bool {
        self.try_is_any_dag_ancestor(list, queried).unwrap()
    }

    /// Result version of [`Self::is_any_dag_ancestor`] (avoids unwrapping internally)
    fn try_is_any_dag_ancestor(&self, list: &mut impl Iterator<Item = BlockHash>, queried: BlockHash) -> Result<bool>;
    ...
    fn get_next_chain_ancestor(&self, descendant: BlockHash, ancestor: BlockHash) -> BlockHash;

    /// Returns the chain parent of `this`
    fn get_chain_parent(&self, this: BlockHash) -> BlockHash;

    /// Checks whether `this` has reachability data
    fn has_reachability_data(&self, this: BlockHash) -> bool;
}
...
impl<T: ReachabilityStoreReader + ?Sized> MTReachabilityService<T> {
    ...
    pub fn default_backward_chain_iterator(&self, from: BlockHash) -> impl Iterator<Item = BlockHash> {
...
impl<T: ReachabilityStoreReader + ?Sized> BackwardChainIterator<T> {
    fn new(store: Arc<RwLock<T>>, from_descendant: BlockHash, to_ancestor: BlockHash, inclusive: bool) -> Self {
...
impl<T: ReachabilityStoreReader + ?Sized> ForwardChainIterator<T> {
    fn new(store: Arc<RwLock<T>>, from_ancestor: BlockHash, to_descendant: BlockHash, inclusive: bool) -> Self {
...
mod tests {
    ...
    fn test_forward_iterator() {
        ...
        // Assert
        let expected_hashes = [2u64, 3, 5, 6].map(BlockHash::from);
        ...
        // Assert
        let expected_hashes = [2u64, 3, 5, 6, 10].map(BlockHash::from);
        assert!(expected_hashes.iter().cloned().eq(iter));

        // Compare backward to reversed forward
        let forward_iter = service.forward_chain_iterator(2.into(), 10.into(), true);
        let backward_iter: Vec<BlockHash> = service.backward_chain_iterator(10.into(), 2.into(), true).collect();
    ...
    fn test_iterator_boundaries() {
        ...
        // Asserts
        assert!([1u64, 2].map(BlockHash::from).iter().cloned().eq(service.forward_chain_iterator(1.into(), 2.into(), true)));
        assert!([1u64].map(BlockHash::from).iter().cloned().eq(service.forward_chain_iterator(1.into(), 2.into(), false)));
        assert!([2u64, 1].map(BlockHash::from).iter().cloned().eq(service.backward_chain_iterator(2.into(), root, true)));
        assert!([2u64].map(BlockHash::from).iter().cloned().eq(service.backward_chain_iterator(2.into(), root, false)));
        assert!(std::iter::once(root).eq(service.backward_chain_iterator(root, root, true)));
        assert!(std::iter::empty::<BlockHash>().eq(service.backward_chain_iterator(root, root, false)));
        assert!(std::iter::once(root).eq(service.forward_chain_iterator(root, root, true)));
        assert!(std::iter::empty::<BlockHash>().eq(service.forward_chain_iterator(root, root, false)));
    }
}
