...
use kaspa_consensus_core::palw_state_v2::{
    ...
    apply_palw_transition_v2_with_extras, revert_delta_v2,
};
...
pub struct PalwStateSyncV2 {
    ...
    /// The fused terminal's responder-coverage fence (mainnet audit 2026-09-06, C-2/H-5), carried
    /// for exactly the reason the three above it are — and named by the comment directly below,
    /// which said a rule making the dissection's own arithmetic fence-dependent inside the
    /// transition WOULD need threading. This is that rule.
    court_responder_coverage: Option<kaspa_consensus_core::config::params::ForkActivation>,
    /// ADR-0093 Decision 8's fence, carried for the reason the four above are: past it the fold
    /// refuses a plain root claim where the bottom reads a checkpoint, so a replay that resolved it
    /// differently from the live fold would open a phase the live chain never opened.
    attn_anchored_root: Option<kaspa_consensus_core::config::params::ForkActivation>,
    /// ADR-0107's share-growth fence, carried for the reason the four above are: past it the
    /// boundary's share table depends on it, so a replay that folded without it would write a
    /// different state root for the same block than the live path does.
    share_growth_final: Option<kaspa_consensus_core::config::params::ForkActivation>,
    /// The 2026-09-11 audit fence, carried for the same reason: past it every audited fold site
    /// takes its fixed path, so a replay without it writes a different state root for the block.
    ...
    /// ADR-0144 §9: the HEIGHT of the objective-offence lock ledger. A height, not a per-block
    /// answer, so a replay that left it to `Default` would skip Valid locks the live fold wrote.
    objective_offence: Option<kaspa_consensus_core::config::params::ForkActivation>,
        ...
        for step in steps {
            let (next, delta) = apply_palw_transition_v2_with_extras(
                ...
                &kaspa_consensus_core::palw_state_v2::PalwTransitionExtrasV1 {
                    court_responder_coverage_active: self
                        .court_responder_coverage
