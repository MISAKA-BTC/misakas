...
pub enum PalwStateWalkError {
    ...
    NoAnchor,
    #[error("the stored anchor stands at {stored}, but the walk was asked to start at {asked}")]
    AnchorMismatch { stored: BlockHash, asked: BlockHash },
    #[error("block {0} has no stored PALW delta — a missing delta is an error, never an empty one")]
    MissingDelta(BlockHash),
    #[error("the stored delta for block {block} does not decode: {source}")]
    UndecodableDelta { block: BlockHash, source: std::io::Error },
    #[error("the stored anchor snapshot does not decode: {0}")]
    UndecodableAnchor(std::io::Error),
...
pub fn load_anchor(store: &DbPalwStateV2Store, params: &PalwStateParamsV2) -> Result<(BlockHash, PalwChainStateV2), PalwStateWalkError> {
    let record = store.anchor()?.ok_or(PalwStateWalkError::NoAnchor)?;
    let carriage = record.decode().map_err(PalwStateWalkError::UndecodableAnchor)?;
    let state = carriage.into_state(params, Some(record.state_root))?;
    Ok((record.block, state))
}
...
fn load_delta(
    ...
    let row = match store.delta_row(block) {
        Ok(row) => row,
        ...
        // half-written history look like a valid chain with less work on it.
        Err(kaspa_database::prelude::StoreError::KeyNotFound(_)) => return Err(PalwStateWalkError::MissingDelta(block)),
        Err(e) => return Err(e.into()),
    };
    row.decode().map_err(|source| PalwStateWalkError::UndecodableDelta { block, source })
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::model::stores::palw_state_v2::PalwStateAnchorRecord;
    use kaspa_consensus_core::palw_state_v2::{
        PalwBlockContextV2, PalwBlockWorkV3, PalwBondKeyV2, PalwClassDaaV2Params, PalwConsensusObjectV2, PalwPanelSeatV2,
        PalwPwuRuleV2, PalwStateBookV2, PalwStateCarriageV2, apply_palw_transition_v3,
    };
    ...
    fn ctx(block: u64, daa: u64, blue: u64) -> PalwBlockContextV2 {
        PalwBlockContextV2 { block: h64(block), daa_score: daa, blue_score: blue }
    }
    ...
    fn registrations() -> Vec<PalwConsensusObjectV2> {
        vec![
            ...
            PalwConsensusObjectV2::BondRegistered { bond: bond(), pubkey: vec![7; 4], operator_id: h64(21), collateral: 100_000 },
        ]
    }
    ...
    fn fixture() -> (
        ...
        let mut persist = |block: BlockHash, delta: &kaspa_consensus_core::palw_state_v2::PalwStateDeltaV2| {
            let mut batch = WriteBatch::default();
            store.insert_delta_batch(&mut batch, block, delta).unwrap();
        ...
        let steps: Vec<(PalwBlockContextV2, Vec<PalwConsensusObjectV2>)> = vec![
            ...
            (ctx(4, 103, 4), vec![PalwConsensusObjectV2::ReceiptLicensed { claim: h64(0xFC) }]),
            (ctx(5, 124, 5), vec![]),
        ];
    ...
    fn the_store_walk_and_the_in_memory_book_agree() {
        ...
        book.apply_block(h64(2), ctx(3, 102, 3), &[PalwConsensusObjectV2::PanelBound { claim: h64(0xFC), anchor: h64(77), seats }], None)
            .unwrap();
        book.apply_block(h64(3), ctx(4, 103, 4), &[PalwConsensusObjectV2::ReceiptLicensed { claim: h64(0xFC) }], None).unwrap();
    ...
    fn the_anchor_is_the_walks_starting_point_and_is_verified() {
        ...
        let root = fork_state.state_root();
        let carriage = PalwStateCarriageV2::from_state(&fork_state);
        let mut batch = WriteBatch::default();
        store.set_anchor_batch(&mut batch, PalwStateAnchorRecord::encode(fork, &carriage, root)).unwrap();
        db.write(batch).unwrap();
        ...
        // A snapshot whose root no longer matches cannot load.
        let mut tampered = carriage;
        tampered.safe_weight += 1;
        let mut batch = WriteBatch::default();
        store.set_anchor_batch(&mut batch, PalwStateAnchorRecord::encode(fork, &tampered, root)).unwrap();
        db.write(batch).unwrap();
        assert!(matches!(load_anchor(&store, &p), Err(PalwStateWalkError::State(_))), "a tampered anchor cannot load");
    }
}
