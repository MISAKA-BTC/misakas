...
impl VirtualStateProcessor {
    ...
    pub(super) fn verify_expected_utxo_state<V: UtxoView + Sync>(
        ...
        // ADR-0134: past the compute overlay's retirement none of its five subnetworks enters a
        // block. Below it they are accepted and do nothing — the machinery that once read them (the
        ...
        let carve = self.fee_split_at(header.daa_score);
    ...
    /// **ADR-0134: past `Params::palw_compute_overlay_retired` no compute-overlay transaction
    /// enters a block** — a certificate, challenge, capability, commitment or verdict (the VLT's
