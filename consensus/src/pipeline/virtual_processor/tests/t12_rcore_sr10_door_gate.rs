...
async fn t12_the_gate_routes_a_set_on_a_licensed_claim_to_the_v3_door() {
    ...
    // and locked with their own masks and the cut, the recount 2 (every segment: the full seat
    // and its holder), their served bits set.
    ...
    assert_eq!(record.rcore.basis_k, 2, "the licence's three and the door's two recount to 2");
    assert_eq!(
