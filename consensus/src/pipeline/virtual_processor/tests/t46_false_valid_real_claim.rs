...
fn assert_convicted_before_final(
    ...
    for &card in convicted {
        ...
        // The Phase 1–2 review's L-2 (ADR R-2): what the conviction took, and the claim.
        assert_eq!((row.collected as u128, row.claim_id), (lock.amount, claim_id), "card {card}: collected = the debit, the claim");
    }
    assert!(
...
fn assert_refuted_before_final(
    ...
    // The Phase 1–2 review's L-2 (ADR R-2): what the conviction took, and the claim.
    assert_eq!((row.collected, row.claim_id), (debit, claim_id), "collected = the debit, and the claim");
