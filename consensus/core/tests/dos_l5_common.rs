...
/// `bound + 13,996`) are the ones a 2M claim will meet. testnet-12's own pruning depth already holds
/// the lattice at D_cap (P-1), which a ruleset with a measured row must (E-11), so nothing else moves.
pub fn t12_2m_open() -> Params {
    let mut p = t12();
    p.palw_class_verify_rows = Box::leak(Box::new([t12_2m_flag_day_row()]));
    p.sync_palw_class_verify_deadline();
    p.validate_palw_v2().expect("the flag-day fixture validates on testnet-12's own horizon");
    p
}
