...
fn run(audit: bool) {
    ...
    let equivocation = equivocation_carriage(&accused.0, &profile);
...
fn sibling_spend_after_conviction(audit: bool) -> Result<PalwChainStateV2, PalwStateV2Error> {
    ...
    let payload = kaspa_consensus_core::palw_offence_v1::PalwPanelFalseValidEvidenceV1 {
        ...
        executor_pubkey: pk.clone(),
        contradiction: kaspa_consensus_core::palw_offence_v1::PalwPanelContradictionV1::ExecutorEquivocation(equivocation_carriage(&accused.0, &profile)),
    };
    let evidence = borsh::to_vec(&payload).unwrap();
    let evidence_id = kaspa_consensus_core::palw_offence_v1::palw_offence_evidence_digest_v1(&evidence);
    let s = fold_at(
...
/// A minimal `PalwEquivocationCarriageV1` for the ExecutorEquivocation contradiction. Its content
/// is never validated on the PanelFalseValid conviction path
/// (`palw_panel_contradiction_convicts_execution_v1` returns Ok for ExecutorEquivocation, and
/// `bind_panel_false_valid` is a no-op for it) — the point being demonstrated is exactly that a
/// conviction lands and slashes the lock without ever touching the claim's phase.
