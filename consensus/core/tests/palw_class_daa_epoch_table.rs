...
fn difficulty_and_share_co_adjust_across_models_to_equilibrium() {
    ...
    for epoch in 0..EPOCHS {
        ...
        let plan: std::collections::BTreeMap<Hash64, u64> = [(fast, FAST_CAPACITY), (heavy, HEAVY_CAPACITY)]
            ...
            .map(|(class, capacity)| {
                ...
                let budget =
                    chain.state.epoch_budgets().and_then(|b| b.budget_blocks.get(&class).copied()).unwrap_or(0);
        ...
        loop {
            ...
            let class = [fast, heavy]
                ...
                .find(|c| {
                    ...
                    did < want && (chain.daa % (EPOCH_LENGTH / want.max(1).min(EPOCH_LENGTH))) == 0
                })
                .unwrap_or(base);
        ...
        let produced: std::collections::BTreeMap<Hash64, u64> = [base, fast, heavy]
            .iter()
            .map(|c| {
                (*c, chain.state.epoch_counter(c).filter(|k| k.epoch_index == closed).map(|k| k.produced_blocks).unwrap_or(0))
            })
            .collect();
    ...
    assert!(
        fast_share >= 25 && fast_share <= 60,
        "the fast model should stall near its 40-win capacity, got {fast_share}‰"
    );
    assert!(
    ...
    assert!(
        heavy_share < heavy_share_at_silence,
        "a silent model's share must decay ({heavy_share_at_silence}‰ -> {heavy_share}‰)"
    );
