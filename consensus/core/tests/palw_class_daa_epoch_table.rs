...
use kaspa_consensus_core::palw_state_v2::{
    apply_palw_transition_v2, PalwBlockContextV2, PalwBondKeyV2, PalwChainStateV2, PalwConsensusObjectV2 as Obj, PalwPwuRuleV2,
    PalwStateParamsV2,
};
...
fn the_real_qwen36_class_earns_share_by_producing() {
    ...
    let p = bundle.state.clone().with_class_share_growth_v1(250, 500).expect("the growth params are in range");
    let epoch_length = p.epoch_length();
...
fn the_share_rule_fires_only_where_it_should() {
    ...
    let build = |growth: u16| {
        ...
        if growth > 0 {
            p = p.with_class_share_growth_v1(growth, 500).expect("in range");
        }
