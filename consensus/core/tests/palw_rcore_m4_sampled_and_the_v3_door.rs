...
/// A REAL S2 licence of the full seat and one partial at `L`: in this tree the licence arms stage
/// no R-core+ record, so `claim.rcore` is its default — the state the door actually meets today.
...
/// **The door on the record a real S2 licence leaves in this tree.** S-2 has not landed, so the
/// licence stages no R-core+ record (`rcore` is default: no door, `basis_k` 0, no served bit) and
/// `license_claim` already ticked the anchor. The door then recounts the counted locks (the full
/// seat's mask-less lock reads as its assigned full mask) with the three new partial `Valid`s:
/// `basis_k` 2, no door recorded, no second tick, served bits for the door's seats only, and the
/// claim still counts as licensed for the room (no door recorded). S-2's licence staging changes
/// the first three facts on purpose; this test fails then and is re-derived with it.
#[test]
fn the_door_on_a_real_pre_s2_s2_licence() {
    let p = params(true);
    let (s2, _) = bound(&p);
    let (s3, claim_id, full, partial) = s2_licensed(&p);
    assert_eq!(s3.claim(&claim_id).unwrap().rcore, PalwClaimRcoreV1::default(), "S-2 not landed: the licence stages nothing");
    assert_eq!(s3.settled_attempt_finals(), s2.settled_attempt_finals() + 1, "pre-S-2 the S2 licence ticks in license_claim");
    let lock = *s3.slashable_lock(full, claim_id).expect("the full seat locked at licence");
    assert_eq!(lock.segments, 0, "mask-less: the licence arms record no mask yet");
    ...
    assert_eq!(claim.rcore.basis_k, 2, "full (assigned full) + p0 + p1..p3: every segment twice");
    assert_eq!(claim.rcore.licence_door, None, "no door recorded before S-2, so no upgrade is recorded");
    assert_eq!(s4.settled_attempt_finals(), s3.settled_attempt_finals(), "and no second tick");
    let bits = partial[1..].iter().map(|seat| 1u32 << bonds().iter().position(|b| b == seat).unwrap()).fold(0, |a, b| a | b);
    assert_eq!(claim.rcore.served_mask, bits, "the door's own seats only (the licence's bits are S-2's)");
    assert!(palw_rcore_counts_licensed_v1(&claim));
    assert!(!claim.rcore.escrow_released, "the flip is S-2's seam");
...
fn incapable_on_the_floor_and_an_unassigned_sampled_mask() {
    ...
    assert!(credited(&s4, claim_id, partial[1]) && s4.slashable_lock(partial[1], claim_id).is_none());
    assert_eq!(rcore.served_mask, 0, "no served bit");
