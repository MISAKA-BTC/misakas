...
pub struct VltEpochCredits {
    ...
    /// Audit-emission v0.2's per-verifier audit credit (`docs/misaka-audit-emission-v0.2-design.md`
    /// §2.1). Its only reader was the token overlay's emission, which is removed, and nothing writes
    /// it, so every row carries it empty. It stays because it is the persisted row layout
    /// (`VLT_CREDITS_SCHEMA_VERSION` 4).
