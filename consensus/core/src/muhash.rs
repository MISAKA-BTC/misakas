...
pub trait MuHashExtensions {
    ...
    /// kaspa-pq (ADR-0013 Addendum C.2 / ADR-0016 §D.4): remove a single UTXO
    /// element from the multiset as a consensus side-effect — the multiset
    /// counterpart of [`UtxoDiff::remove_utxo`], used to cancel a slashed
    /// bond's locked output-0 from the `utxo_commitment`. The `entry` must
    /// equal the one added when the UTXO was created (same `block_daa_score`,
    /// amount, spk, coinbase flag) so the element cancels exactly.
