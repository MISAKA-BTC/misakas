...
pub const GENESIS: GenesisBlock = GenesisBlock {
    ...
    hash: Hash64::from_bytes([
        0x3c, 0x8c, 0xf9, 0x33, 0xa1, 0x01, 0xb8, 0x54, 0xaf, 0x3d, 0x03, 0x0f, 0xbb, 0x73, 0x5e, 0x3e, 0x28, 0xe2, 0xf6, 0xa7, 0xe7,
        0x1f, 0x58, 0xb4, 0xc0, 0x4c, 0xca, 0xa4, 0x1f, 0x87, 0x68, 0xd0, 0xb4, 0xeb, 0xff, 0xf2, 0x17, 0x3d, 0x6e, 0x14, 0x9b, 0x7d,
        0x69, 0x06, 0xcc, 0xb4, 0x36, 0xc1, 0xf3, 0x76, 0xda, 0x47, 0x0b, 0xe5, 0x26, 0x7f, 0x79, 0x2e, 0xef, 0x80, 0xe8, 0x6e, 0x02,
        0xd7,
    ]),
    version: 0,
...
pub const TESTNET_GENESIS: GenesisBlock = GenesisBlock {
    hash: Hash64::from_bytes([
        0x4f, 0x0f, 0x27, 0x7d, 0xb6, 0x0a, 0x50, 0xce, 0x3e, 0x61, 0xdc, 0xc9, 0x0a, 0x33, 0xd9, 0x34, 0x12, 0x34, 0xcb, 0x15, 0xd6,
        0x39, 0xe0, 0x45, 0x52, 0x1d, 0x1d, 0x62, 0x9c, 0xd5, 0x25, 0x5b, 0xcc, 0xf2, 0x14, 0x88, 0xdd, 0x8c, 0xbf, 0x48, 0xc7, 0xcd,
        0xc3, 0x61, 0xd4, 0x35, 0x8a, 0x68, 0xc1, 0xac, 0x04, 0x7d, 0x30, 0x01, 0x9a, 0x85, 0x7a, 0x37, 0x61, 0xc6, 0x64, 0x85, 0x91,
        0xea,
    ]),
    version: 0,
    // PR-9.5g: recomputed (64-byte Hash64) via `gen_kaspa_pq_genesis_hashes`.
    ...
    timestamp: 1748390400000,
    // kaspa-pq Phase 3 (ADR-0007): PoW is now the compute-only BLAKE2b-512 ∥ SHA3-512
    // Layer-1 (`pow_blake2b_sha3_activation = always`, `algo_id = 3`), ~10^4× faster than
    // the Phase-2 Argon2id this `0x20018618` (D ≈ 84) value was calibrated against. So at
    // the new hash-rate the genesis difficulty is ~10^4× too easy; it is kept as the EASY
    // launch floor because erring easy is self-correcting (the DAA ramps live difficulty up
    // to the hash-rate equilibrium D ≈ aggregate-H/s ÷ BPS within the first
    // ~MIN_DIFFICULTY_WINDOW_SIZE blocks; floors at `max_difficulty_target`, no upper cap)
    // and never stalls. Mine UN-throttled so difficulty rises to equilibrium (a throttle
    // pins the observed rate and starves the DAA, leaving difficulty pinned low). Re-bench
    // the BLAKE2b-SHA3 hash-rate with `pq-miner --bench-secs` and pre-set this nearer the
    // equilibrium to skip the initial instamine ramp (the first window is fixed-difficulty).
    bits: 0x20018618,
    nonce: 0,
    daa_score: 0,
    #[rustfmt::skip]
