...
//
// `utxo_commitment` commits to the kaspa-pq genesis premine: **exactly the 10B cap**
// (re-genesis 2026-08-30) — one main-wallet UTXO, from which testnet-11 carves its
// genesis-bond collateral, fee floats and community allocation — each UTXO locked
// to a single-key ML-DSA-87 P2PKH (see `config::premine::genesis_premine_utxos_for`).
// It is the MuHash over that UTXO set. Per audit H-01 the main wallet is
// network-dependent — mainnet uses the operator custody address, testnet-11 the
// operator's public PALW address, the other test networks the Claude-managed test
// key — so the mainnet `utxo_commitment`/`hash` differ from testnet/devnet/simnet.
// The premine UTXOs themselves are imported into the UTXO store at consensus init
...
pub const GENESIS: GenesisBlock = GenesisBlock {
    ...
    hash: Hash64::from_bytes([
        0xe4, 0xe8, 0x24, 0x72, 0x0b, 0x09, 0xd9, 0xc5, 0xd6, 0x7f, 0x13, 0x9a, 0xbd, 0xdf, 0x8e, 0x7e, 0xa7, 0x7f, 0x3e, 0xda, 0xf4,
        0xc5, 0x8c, 0x34, 0x17, 0xd6, 0x54, 0xab, 0xbf, 0x9d, 0xdd, 0x36, 0xe5, 0xed, 0xdb, 0x4b, 0xfe, 0x90, 0x35, 0x22, 0xab, 0x77,
        0xba, 0x03, 0x3b, 0xc7, 0x0d, 0x8e, 0xb5, 0xc8, 0xcd, 0xa6, 0x31, 0xa1, 0x20, 0x5b, 0xef, 0x04, 0xd9, 0xa9, 0xd1, 0x0a, 0x02,
        0xc8,
    ]),
    version: 0,
    ...
    // kaspa-pq (audit H-01): genesis commits to the 10B cap (re-genesis 2026-08-30:
    // one main-wallet UTXO, no vault block) = MuHash over `misaka_premine_utxos(Mainnet)`.
    // Mainnet's 10B main wallet is the operator custody address (ceremony complete), so
    // this commitment differs from the test networks (whose main wallet is the
    // Claude-managed test key, or the operator's public PALW address on testnet-11).
    utxo_commitment: Hash64::from_bytes([
        0x53, 0xc8, 0xc0, 0xb6, 0x3e, 0xac, 0xf8, 0x85, 0x92, 0x2c, 0x47, 0x49, 0xce, 0xda, 0x67, 0xae, 0x92, 0x94, 0x55, 0x6e, 0x75,
        0x62, 0xfa, 0xfa, 0xb3, 0x02, 0xf6, 0x68, 0xe1, 0xcf, 0x1b, 0x23, 0xe7, 0xea, 0xed, 0xd5, 0x41, 0xcf, 0x1c, 0x67, 0x5c, 0x5e,
        0x17, 0x20, 0xca, 0x36, 0x8b, 0x25, 0x36, 0xe1, 0x02, 0x54, 0xd7, 0x29, 0x72, 0x9a, 0x5f, 0x0f, 0xb4, 0x91, 0x5d, 0x13, 0x6a,
        0xe9,
    ]),
...
pub const TESTNET_GENESIS: GenesisBlock = GenesisBlock {
    ...
    hash: Hash64::from_bytes([
        0x9b, 0x9b, 0xfe, 0x36, 0x16, 0xd5, 0x7e, 0xe7, 0x3f, 0x9e, 0x2a, 0xf5, 0xf9, 0xf1, 0x41, 0x1e, 0x68, 0xba, 0x86, 0xf2, 0x4d,
        0x13, 0x4e, 0x06, 0x27, 0xf2, 0x58, 0x54, 0x0c, 0xd0, 0x53, 0x45, 0x65, 0x9a, 0xc3, 0x73, 0xb9, 0x13, 0x57, 0x53, 0x62, 0xc9,
        0x39, 0x1f, 0x42, 0xf7, 0xd7, 0x1a, 0x4d, 0x6c, 0xf1, 0x15, 0xd1, 0xc5, 0xe4, 0x28, 0xd0, 0x74, 0x3b, 0x29, 0x22, 0xce, 0xe1,
        0x23,
    ]),
    version: 0,
    // PR-9.5g: recomputed (64-byte Hash64) via `gen_kaspa_pq_genesis_hashes`.
    ...
    // kaspa-pq: genesis commits to the 10B cap (re-genesis 2026-08-30: one main-wallet
    // UTXO, no vault block) = MuHash over `config::premine::misaka_premine_utxos()`.
    // Test nets share one commitment (same Claude-managed main wallet); mainnet differs.
    utxo_commitment: Hash64::from_bytes([
        0x5d, 0xd7, 0x1c, 0x0b, 0xad, 0x0a, 0xee, 0x22, 0x9f, 0x4d, 0x47, 0x84, 0x00, 0x39, 0xd4, 0x0f, 0xae, 0x31, 0xb8, 0xde, 0x23,
        0xec, 0xfd, 0xce, 0x14, 0xed, 0x15, 0xa7, 0x98, 0x33, 0x34, 0x53, 0xb7, 0xe2, 0x17, 0x47, 0x27, 0xd1, 0x01, 0xf4, 0x3a, 0xef,
        0xed, 0x96, 0x8f, 0xe2, 0xf7, 0x11, 0x5a, 0xcd, 0xfd, 0x22, 0xaf, 0xd4, 0x91, 0xf4, 0xa7, 0x77, 0xe9, 0xc0, 0xc8, 0xf8, 0xb6,
        0x0b,
    ]),
    timestamp: 1748390400000,
...
pub const TESTNET11_GENESIS: GenesisBlock = GenesisBlock {
    ...
    hash: Hash64::from_bytes([
        0x57, 0x2f, 0x80, 0xc0, 0x64, 0xba, 0x55, 0x39, 0x61, 0x8d, 0xaf, 0x3c, 0x7e, 0x3e, 0xaa, 0xee, 0x70, 0x20, 0x22, 0x13, 0xe7,
        0x58, 0x46, 0x19, 0xc6, 0xed, 0x88, 0x82, 0xae, 0x17, 0x68, 0x86, 0xa9, 0x53, 0xfa, 0xf6, 0x98, 0xaf, 0x58, 0x31, 0x63, 0x3e,
        0x86, 0xa4, 0xa3, 0x87, 0x46, 0xd5, 0x4d, 0x83, 0xc5, 0x6f, 0xe8, 0x25, 0x14, 0x37, 0x3f, 0xfa, 0x00, 0x56, 0x09, 0xdc, 0x6c,
        0xd4,
    ]),
    hash_merkle_root: Hash64::from_bytes([
        0x77, 0x95, 0x70, 0x85, 0x9f, 0x02, 0xb3, 0x0d, 0xfa, 0x2e, 0x89, 0x31, 0x16, 0x87, 0xb9, 0x72, 0xa1, 0x27, 0x26, 0x1f, 0xbe,
        0x6f, 0x36, 0x5a, 0xcb, 0x17, 0xc7, 0xdf, 0x1f, 0xb0, 0x24, 0x91, 0x6e, 0x45, 0x06, 0x0b, 0xe0, 0x24, 0xb4, 0xd3, 0xa1, 0x1e,
        0xeb, 0x2c, 0xcd, 0xb1, 0x51, 0x1c, 0x4b, 0x89, 0x02, 0x73, 0xdf, 0x16, 0xfb, 0xba, 0x66, 0x36, 0xf7, 0x50, 0xf5, 0x7f, 0xe2,
        0x66,
    ]),
    // The arm-ready re-genesis (2026-08-31, "Relaunch 4"): commits to EXACTLY 10B MSK = the main
    // wallet, minus the carve-outs it pays for — EIGHT 10,000-MSK genesis-bond collateral outputs (ADR-0061) (at
    // the bonds' own outpoint indices, owned by the main wallet's key), eight 100-MSK fee floats,
    // and the 547M community allocation (`config::premine::TESTNET11_COMMUNITY_ALLOCATIONS`,
    // 11 UTXOs on the "misaka-t11-community" sentinel txid) = MuHash over
    // `config::premine::genesis_premine_utxos_for(testnet-11)`. testnet-10 keeps the shared
    // test-net commitment above.
    utxo_commitment: Hash64::from_bytes([
        0x7f, 0x31, 0x42, 0xf2, 0x1c, 0xd8, 0x29, 0x85, 0xc4, 0x4d, 0x4d, 0x46, 0x57, 0xfd, 0x83, 0x07, 0xfe, 0xb3, 0xee, 0xa8, 0xd3,
        0xcc, 0x3e, 0xd3, 0x17, 0x45, 0x66, 0xeb, 0x04, 0x09, 0x77, 0x69, 0xe9, 0x11, 0x85, 0x7a, 0xaf, 0x59, 0xd8, 0xdd, 0x7b, 0xe9,
        0xe9, 0x50, 0x9d, 0xce, 0x6b, 0xd9, 0x61, 0xe5, 0x72, 0xd6, 0xfc, 0x01, 0xb8, 0xb2, 0xb4, 0xd5, 0xcf, 0xdf, 0xdd, 0x7d, 0x93,
        0x03,
    ]),
    ...
    coinbase_payload: &[
        ...
        // "misaka-testnet"
        0x6d, 0x69, 0x73, 0x61, 0x6b, 0x61, 0x2d, 0x74, 0x65, 0x73, 0x74, 0x6e, 0x65, 0x74,
        // TN11, kaspa-pq Relaunch 4: the arm-ready re-genesis (2026-08-31 — the bond registry
        // grown from six seats to eight so ADR-0065 D1 can be armed as a config change instead of
        // a re-mint; Relaunch 3 was the 10B-cap re-genesis of 2026-08-30). The bump makes this
        // chain cryptographically distinct from its predecessor — an un-wiped node hits the
        // startup genesis-mismatch guard instead of silently resuming.
        11, 4,
    ],
    ..TESTNET_GENESIS
};
...
pub const SIMNET_GENESIS: GenesisBlock = GenesisBlock {
    hash: Hash64::from_bytes([
        0x30, 0xfb, 0xcb, 0x2d, 0xc2, 0x50, 0xe8, 0x74, 0x27, 0x33, 0x4d, 0xd2, 0x09, 0xfa, 0x9f, 0x72, 0x57, 0x39, 0x1e, 0x81, 0x38,
        0x71, 0x63, 0xae, 0x4a, 0xec, 0x57, 0x1a, 0xac, 0x94, 0x9f, 0xf7, 0x44, 0x4d, 0xd2, 0x00, 0xa5, 0x83, 0x64, 0x1b, 0x3d, 0x14,
        0x9a, 0x1c, 0xdb, 0x9d, 0x02, 0x4f, 0xba, 0xef, 0x75, 0x49, 0x58, 0x02, 0xf1, 0xaf, 0x3a, 0x99, 0x8e, 0xda, 0x6a, 0xe5, 0x99,
        0x73,
    ]),
    version: 0,
    // PR-9.5g: recomputed (64-byte Hash64) via `gen_kaspa_pq_genesis_hashes`.
...
pub const PALW_RC_GENESIS: GenesisBlock = GenesisBlock {
    hash: Hash64::from_bytes([
        0xad, 0x30, 0xb5, 0xcb, 0x96, 0x5a, 0xd3, 0x05, 0xdf, 0xa1, 0xdc, 0x75, 0x16, 0x93, 0x57, 0x63, 0xea, 0x26, 0x23, 0x10, 0x55,
        0x81, 0xb8, 0x3b, 0xb9, 0x35, 0x9c, 0x72, 0x47, 0x15, 0x7d, 0x36, 0xb0, 0xf8, 0x00, 0x3b, 0x33, 0x7c, 0xda, 0xd3, 0x66, 0xe3,
        0x89, 0x5c, 0x8f, 0x15, 0x9e, 0x99, 0x33, 0x2b, 0xe1, 0x6e, 0x25, 0x8b, 0x14, 0x4d, 0xdd, 0xf4, 0x83, 0xbf, 0x9b, 0x33, 0xed,
        0xb7,
    ]),
    version: 0,
    hash_merkle_root: Hash64::from_bytes([
    ...
    // the 9B main wallet differs, and since 2026-08-22 it is ALSO reduced by the eight genesis-bond
    ...
    utxo_commitment: Hash64::from_bytes([
        0xba, 0x26, 0x12, 0x41, 0x7e, 0x7e, 0x08, 0x17, 0xcc, 0xa0, 0xac, 0x0c, 0xad, 0xe9, 0x1c, 0xaa, 0x58, 0x58, 0x34, 0xc0, 0x51,
        0x11, 0x4b, 0xc4, 0x12, 0x55, 0x42, 0xac, 0xb0, 0x58, 0x98, 0xdb, 0xb8, 0x69, 0xbe, 0x31, 0xca, 0xd0, 0x2a, 0xbc, 0xd9, 0xdd,
        0xfd, 0x62, 0xde, 0x08, 0x23, 0x39, 0x19, 0xf2, 0x66, 0x5f, 0xe8, 0xd7, 0xfb, 0x3d, 0xaa, 0xda, 0x2b, 0x0f, 0xf4, 0xb2, 0xc5,
        0xaa,
    ]),
    timestamp: 1748390400000,
    bits: 0x207fffff,
...
/// **testnet-12's genesis — the 2026-09-22 regenesis of testnet-11.**
///
/// A NEW chain, and every field below says so:
///
/// * **The coinbase marker is `misaka-palw-t12`**, which moves the merkle root and therefore the
///   block hash. That is what makes an un-wiped t11 datadir hit the startup genesis-mismatch guard
///   instead of silently resuming on a chain whose rules changed underneath it — the regenesis is
///   announced by the block, not by a release note.
/// * **The `utxo_commitment` is t12's own premine**: the same 10B cap, the same eight genesis-bond
///   collateral outputs and fee floats (the operators keep their keys), and the **858M t12 community
///   allocation** ([`config::premine::TESTNET12_COMMUNITY_ALLOCATIONS`], 17 UTXOs) = MuHash over
///   `config::premine::genesis_premine_utxos_for(testnet-12)` — **on testnet-12's own txids**
///   (`premine_txid_for(testnet-12)` and `testnet12_community_txid()`, derived from the sentinels,
///   the network id and `PALW_T12_PREMINE_SALT`; user decision 2026-09-24, replay separation): the
///   private testnet-12 instances minted the same indices on the shared sentinels with the same
///   card keys, and a spend signed there must not be a spend here.
/// * **Its own timestamp** (2026-09-01T00:00:00Z), so the block hash differs from every chain that
///   shared the reference timestamp even apart from the commitment.
/// * Everything else structural is [`PALW_RC_GENESIS`]'s: `bits` at the easiest representable
///   target (one attempt is one full pinned-LLM inference, so a hash-miner's start bits would never
///   find a block), `daa_score` 0.
///
/// The `hash`, `hash_merkle_root` and `utxo_commitment` below are COMPUTED
/// (`gen_kaspa_pq_genesis_hashes` and `config::premine::tests::print_premine_commitment`), not
/// typed, and `every_shipped_genesis_commits_to_its_own_premine` refuses a build whose commitment
/// and premine have drifted apart.
pub const PALW_T12_GENESIS: GenesisBlock = GenesisBlock {
    hash: Hash64::from_bytes([
        0xa2, 0x7f, 0x8f, 0x44, 0xfe, 0x4d, 0x91, 0xa5, 0xbe, 0xd9, 0x40, 0xbe, 0x9d, 0xbd, 0x6d, 0x26, 0x0c, 0xcb, 0x95, 0xcc, 0x00,
        0xd9, 0x48, 0xb1, 0xc0, 0x8d, 0xdb, 0x6b, 0xd1, 0xa5, 0xf0, 0x25, 0x42, 0xa6, 0xcf, 0x35, 0xc7, 0xa4, 0xd9, 0x59, 0xba, 0x48,
        0x63, 0xac, 0x15, 0x57, 0x86, 0x16, 0x71, 0x76, 0x3e, 0x5c, 0xc2, 0x29, 0x37, 0xc6, 0x97, 0x87, 0x02, 0x83, 0xa8, 0xca, 0x1f,
        0x23,
    ]),
    hash_merkle_root: Hash64::from_bytes([
        0x5e, 0xf0, 0x4d, 0x1b, 0x9a, 0x6c, 0xb0, 0x9e, 0x72, 0x8a, 0x97, 0x0d, 0x2e, 0x0b, 0x75, 0xa0, 0x14, 0x2e, 0xca, 0xcc, 0x4a,
        0x37, 0x3c, 0x9b, 0x71, 0x9e, 0x47, 0xd6, 0xa0, 0xd2, 0x05, 0x8c, 0xb0, 0x48, 0xfa, 0x24, 0x0e, 0x13, 0xf1, 0xbb, 0xb0, 0x94,
        0xdd, 0x4e, 0x19, 0x59, 0x19, 0xea, 0x04, 0x1d, 0xb2, 0xee, 0x33, 0x8f, 0x21, 0xde, 0x34, 0x36, 0x4f, 0x88, 0x53, 0x17, 0xf8,
        0x05,
    ]),
    utxo_commitment: Hash64::from_bytes([
        0x6e, 0xe5, 0x15, 0x71, 0xa6, 0x51, 0x64, 0xfb, 0x60, 0xda, 0xd8, 0x42, 0x8e, 0x4a, 0x0f, 0x42, 0x58, 0x4e, 0x84, 0x53, 0xbf,
        0x6d, 0x02, 0x90, 0x38, 0x45, 0x9e, 0xfe, 0x92, 0x79, 0xb2, 0x65, 0xad, 0x47, 0x0e, 0xe5, 0xc4, 0x2f, 0x2d, 0x90, 0x45, 0x0c,
        0xe3, 0x78, 0x24, 0x41, 0x85, 0x90, 0xb3, 0x0f, 0xe3, 0x61, 0x93, 0xcc, 0x37, 0xd2, 0x03, 0x8f, 0xcf, 0x75, 0xec, 0xb8, 0x4e,
        0xed,
    ]),
    #[rustfmt::skip]
    coinbase_payload: &[
        ...
        0x01,       // Varint
        0x00,       // OP-FALSE
        // "misaka-palw-t12" — the marker that makes this block, and therefore this chain,
        // cryptographically distinct from testnet-11's.
        0x6d, 0x69, 0x73, 0x61, 0x6b, 0x61, 0x2d, 0x70, 0x61, 0x6c, 0x77, 0x2d, 0x74, 0x31, 0x32,
    ],
    // 2026-09-01T00:00:00Z: testnet-12's own (replay separation), not the shared reference. In the
    // past on purpose: a genesis dated after the wall clock refuses the first blocks as "too far
    // into the future", and a harness that mines from the genesis time needs the headroom.
...
pub const DEVNET_GENESIS: GenesisBlock = GenesisBlock {
    ...
    hash: Hash64::from_bytes([
        0xd2, 0x39, 0xd8, 0xbf, 0x48, 0x90, 0x91, 0x55, 0x54, 0xfa, 0x05, 0xe8, 0xff, 0x33, 0x4e, 0x53, 0x0f, 0x57, 0x3b, 0xe6, 0x14,
        0xb2, 0x25, 0x9f, 0x19, 0xad, 0x4f, 0x3d, 0xbe, 0xa9, 0x39, 0xcf, 0xfe, 0x42, 0xce, 0xd0, 0x26, 0xd3, 0xfb, 0xe9, 0x36, 0xdf,
        0xa7, 0x43, 0x34, 0xb4, 0x81, 0x94, 0x09, 0x7c, 0xa6, 0x19, 0x04, 0x13, 0xa3, 0xea, 0xd7, 0xd7, 0xe5, 0x91, 0x4e, 0x52, 0x5c,
        0xa8,
    ]),
    version: 0,
    // PR-9.5g: recomputed (64-byte Hash64) via `gen_kaspa_pq_genesis_hashes`.
    ...
    // Test nets share one commitment (same Claude-managed main wallet); mainnet differs.
    utxo_commitment: Hash64::from_bytes([
        0xdf, 0x60, 0xc9, 0xdf, 0xc2, 0x66, 0x97, 0xdc, 0xb8, 0x78, 0xb2, 0x58, 0x32, 0x04, 0x1c, 0xfa, 0xcf, 0x6d, 0x02, 0x6a, 0x72,
        0x2d, 0xe8, 0xe2, 0x10, 0x9c, 0xac, 0x70, 0x50, 0x2f, 0x50, 0x40, 0xc3, 0xfb, 0x35, 0x8f, 0xff, 0x5a, 0xdb, 0xc7, 0xef, 0xa5,
        0x3b, 0xcc, 0x96, 0xec, 0x31, 0x2d, 0x04, 0x95, 0x0b, 0x99, 0x65, 0xdd, 0x08, 0x46, 0x6f, 0x47, 0xf7, 0xbd, 0x73, 0xd6, 0xec,
        0xba,
    ]),
    timestamp: 1748390400000,
...
mod tests {
    ...
    /// **Every shipped genesis commits to the premine this build actually mints.**
    ///
    /// `utxo_commitment` is a hardcoded literal, and the only thing that ever produced it was a
    /// PRINTER (`config::premine::tests::print_premine_commitment`) whose output an operator pasted
    /// here. Nothing compared the two. So a premine change — a value, a vault, an allocation — left
    /// every genesis committing to a UTXO set the code no longer builds, and the first evidence
    /// would have been a node that cannot start, on a chain nobody could join.
    ///
    /// It is the same shape as the mint tool printing ACCEPTED for gates the loader never runs: a
    /// value announced rather than verified. Now the printer is a convenience and this is the
    /// mechanism. Found while reducing the premine from 13B to 10B, which this would have caught
    /// and nothing else would have.
    #[test]
    fn every_genesis_commits_to_the_premine_this_build_mints() {
        use crate::config::premine::genesis_premine_utxos_for;
        use crate::muhash::MuHashExtensions;
        use crate::network::{NetworkId, NetworkType};
        use kaspa_muhash::MuHash;

        // testnet-11's entry is the genesis a node actually SHIPS — `Params::from` routes the
        // suffix to `palw_rc_shipped_params`, whose genesis is `PALW_RC_GENESIS` — not the
        // retired `TESTNET11_GENESIS` fossil (kept only so a node asked for the legacy lane by
        ...
        let cases: [(&str, NetworkId, &GenesisBlock); 6] = [
            ...
            // testnet-12's premine is its own: the 858M community table on the t12 community txid and
        ...
        for (name, net, genesis) in cases {
            let mut ms = MuHash::new();
            for (outpoint, entry) in genesis_premine_utxos_for(net) {
                ms.add_utxo(&outpoint, &entry);
            }
            let computed = ms.finalize();
            if computed != genesis.utxo_commitment {
                wrong.push(format!("  {name}: pinned {}, premine builds {computed}", genesis.utxo_commitment));
            }
        }
        assert!(
            wrong.is_empty(),
            "a genesis commits to a premine this build does not mint:\n{}\n\nRe-run \
             `config::premine::tests::print_premine_commitment` and paste what it prints.",
            wrong.join("\n")
        );
    }
    ...
    fn gen_kaspa_pq_genesis_hashes() {
        for (name, g) in [
            ...
            ("PALW_T12_GENESIS", &PALW_T12_GENESIS),
        ] {
