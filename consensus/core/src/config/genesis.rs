...
pub const GENESIS: GenesisBlock = GenesisBlock {
    ...
    hash: Hash64::from_bytes([
        0x05, 0x62, 0x03, 0xc8, 0xe7, 0x6e, 0xeb, 0xa1, 0x71, 0xa4, 0xb9, 0x62, 0xb2, 0xda, 0xb0, 0x7d, 0xc2, 0x6a, 0x88, 0x37, 0x28,
        0xdb, 0x2f, 0xa8, 0xc7, 0xd5, 0x44, 0x5f, 0xda, 0xb4, 0x8f, 0x3e, 0x85, 0xfd, 0x14, 0xd3, 0x63, 0x9a, 0xbe, 0x30, 0x9e, 0x0e,
        0x07, 0x82, 0x5f, 0x13, 0x25, 0x56, 0x5e, 0x43, 0x5b, 0x63, 0xc2, 0x40, 0x22, 0x67, 0x59, 0xda, 0xbc, 0xdd, 0x29, 0x24, 0x7a,
        0xc1,
    ]),
    version: 0,
    ...
    // kaspa-pq Phase 2 (ADR-0007): PoW migrated to memory-hard Argon2id 16 MiB
    // (`pow_argon2id_activation = always`). Argon2id's hash-rate is ~10^4× lower
    // than kHeavyHash, so the genesis difficulty is eased to `0x1f7fffff`
    // (exp 31, ~512 hashes/block) for a fast Argon2id bootstrap; the DAA then
    // tunes live difficulty from there (and floors at `max_difficulty_target`
    // for a low CPU-hashrate launch). Launch op-team may harden this at the
    // premine ceremony to match expected mainnet hash-rate.
    bits: 0x1f7fffff,
    nonce: 0,
    daa_score: 0,
    #[rustfmt::skip]
...
pub const TESTNET_GENESIS: GenesisBlock = GenesisBlock {
    hash: Hash64::from_bytes([
        0xcf, 0x4c, 0x48, 0xfe, 0xd6, 0xd8, 0xd5, 0x25, 0xa3, 0x3b, 0xb3, 0x3b, 0x9e, 0xd5, 0x6b, 0x57, 0x2e, 0x11, 0x68, 0xe0, 0xb8,
        0xd6, 0xa5, 0xba, 0x32, 0x2c, 0x11, 0xc4, 0x94, 0x87, 0x1a, 0xfb, 0xdf, 0xb1, 0xc0, 0x41, 0x46, 0x5c, 0x9f, 0x40, 0xb6, 0xd6,
        0x1c, 0x72, 0xd7, 0xec, 0x32, 0x89, 0x45, 0x76, 0x34, 0xe6, 0x02, 0xda, 0xbd, 0xf5, 0xfc, 0x96, 0xff, 0xf8, 0x61, 0xb4, 0xcc,
        0xd3,
    ]),
    version: 0,
    // PR-9.5g: recomputed (64-byte Hash64) via `gen_kaspa_pq_genesis_hashes`.
    ...
    timestamp: 1748390400000,
    // kaspa-pq Phase 2 (ADR-0007): genesis difficulty CALIBRATED to the measured
    // testnet Argon2id-16MiB hash-rate so the chain runs at ~10 BPS with NO miner
    // throttle (the DAA then tracks live hash-rate; floors at `max_difficulty_target`,
    // no upper cap — so an exponential rise in miners is absorbed by rising difficulty).
    // The 4-host mesh benches ~1681 H/s aggregate; at ~2 hashes/difficulty the 10-BPS
    // DAA equilibrium is H/20 ≈ 84, hence `0x20018618` (D ≈ 84). Earlier `0x1f7fffff`
    // (D=256) only survived because miners were throttled to ~2 blk/s, which floored
    // the live difficulty to 1; un-throttled mining needs the genesis pre-set near the
    // equilibrium (the first ~10 min are fixed-difficulty, MIN_DIFFICULTY_WINDOW_SIZE).
    bits: 0x20018618,
    nonce: 0,
    daa_score: 0,
    #[rustfmt::skip]
...
pub const TESTNET11_GENESIS: GenesisBlock = GenesisBlock {
    hash: Hash64::from_bytes([
        0xce, 0x03, 0x11, 0xcd, 0x0d, 0xc8, 0x6c, 0x1e, 0xc9, 0x14, 0xe1, 0xaf, 0xfc, 0xcc, 0xbe, 0x9e, 0x9a, 0x59, 0x5f, 0x06, 0x65,
        0xa6, 0xea, 0x05, 0x15, 0x10, 0x53, 0x7c, 0xdb, 0x62, 0x8f, 0x9f, 0x94, 0x7c, 0xac, 0xc6, 0x44, 0x98, 0xdd, 0x51, 0x4c, 0xd8,
        0x56, 0x0c, 0xca, 0x68, 0x20, 0xc3, 0x0c, 0xa8, 0x13, 0x11, 0x4c, 0x61, 0xcc, 0x56, 0xbd, 0xef, 0xdb, 0xda, 0x5b, 0xd2, 0xb2,
        0x9f,
    ]),
    // PR-9.5g: recomputed (64-byte Hash64) via `gen_kaspa_pq_genesis_hashes`.
    hash_merkle_root: Hash64::from_bytes([
    ...
    bits: 522650214, // see `gen_testnet11_genesis` (= testnet target ×10 harder; rescaled after the D≈84 un-throttle calibration)
    #[rustfmt::skip]
    coinbase_payload: &[
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Blue score
        0x00, 0xE1, 0xF5, 0x05, 0x00, 0x00, 0x00, 0x00, // Subsidy
