...
pub fn palw_lifecycle_object_may_ride_v2(object: &PalwConsensusObjectV2) -> Result<(), &'static str> {
    match object {
        ...
        // does. The remaining shape rules (a present signature for 55, a bounded receipt list for
        // 56, the close ceiling for 55) are their owners' to add with the rule (M3, S-5), before M5
        // freezes the layout; a shape refused here is a BLOCK rule, and adding one to a live chain
        // would be a fork, so none is guessed at now.
