//! **ADR-0111 — a seat may demand the committed leaf it needs to judge.** The pure half: which
...
//! picked from (ADR-0111 Decision 3). This module is that bound, spelled from the binding the demand
...
/// **How long a seat waits for the fast path before it demands on chain**, in DAA (ADR-0111
...
/// **A leaf-evidence request rides the interval lane under bit 29** (ADR-0111 Decision 2). Bits 31
...
/// **A leaf's evidence, built by the executor from its own retention** (ADR-0111 Decisions 1 and
