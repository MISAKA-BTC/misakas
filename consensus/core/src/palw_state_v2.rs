...
///
/// **Version 9: ADR-0056 Decisions 3 and 5.** The class record gains `registrant_bond`, the status
...
///
/// **Version 17 (2026-09-02, ADR-0078).** The state gains one table, `derived_artifacts` — the
/// derivations of free-prompt claims, keyed `(claim, transformer)`, rooted after
/// `pending_chunks` — and the object enum gains `DerivedArtifactV1`. A 16 build cannot decode
...
/// **ADR-0080 design A: the digest one chunk of a declared close is pinned by.**
///
/// Its OWN domain rather than [`PALW_STATE_V2_DOMAIN_OBJECT_CHUNK_GROUP`]: that one hashes a
/// WHOLE object and answers "which group is this", while this one hashes one PART and answers
...
pub struct PalwStateParamsV2 {
    ...
    /// ADR-0056 Decision 5: consecutive closed epochs of ZERO production after which an `Active`
...
impl PalwStateParamsV2 {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            // the floor to 1‰), ADR-0056's withdrawn Decision 4 walk, and the share-growth rule's own
    ...
    /// **ADR-0056 Decisions 3 and 5, as one call**: a registry price and a reclamation clock.
    ///
    /// The share WALK is not here. Two branches wrote one, and the one that shipped is
    /// [`Self::with_class_share_growth_v1`]: it measures a class's FILLED BUDGET against the same
    /// closed epoch the retarget reads, where this line's version counted consecutive epochs above
    /// and below two fill thresholds. Filled budget is the better signal for a reason the other
    /// line measured rather than argued — a minimum-share class has `expected == observed == 1`
    /// in the only two states its budget allows, so a streak rule keyed to those states has no
    /// reachable input on exactly the class that most needs to grow.
    ///
    /// A consuming builder rather than two more `new` arguments for the reason every other late
    /// field here is one: a positional list of same-typed integers is a place to transpose two of
    /// them silently.
    pub fn with_class_economy_v1(mut self, registration_exposure_sompi: u64, reclaim_epochs: u32) -> Result<Self, PalwStateV2Error> {
    ...
    /// **The reserve is not an argument.** ADR-0054 Decision 2 asks for a quantity the floor keeps,
    /// and the table already has one: `min_base_class_share_permille`, which the audit added after
    /// showing 999 individually-legal grants could walk the floor to 1‰. A second stored reserve
    /// would be a second place for the same bound to be set, and the two would differ the first
    /// time someone set one of them. So this takes the growth step alone and reads the bound from
    /// the field that owns it — see `base_class_reserve_permille`.
    pub fn with_class_share_growth_v1(mut self, class_growth_permille: u16) -> Result<Self, PalwStateV2Error> {
        ...
        if class_growth_permille > 0 && self.min_base_class_share_permille == 0 {
            return Err(PalwStateV2Error::InvalidParams(
    ...
    /// **The permille the liveness floor keeps** — the reserve a growing class may not draw past
    /// (ADR-0054 Decision 2, ADR-0039 W6′).
    ///
    /// Derived, not stored. The floor is the reservoir every growing class draws from and the one
    /// class every node can run without an artifact; W6′ said its share "may never be zero", and a
    /// rule that moves permille every epoch makes that bound worthless, because a table walked down
    /// to one permille has ended liveness while satisfying the letter of it.
    ///
    /// The quantity is `min_base_class_share_permille`, the field the audit added when it showed
    /// the same walk could be performed by 999 individually-legal GRANTS. Growth and granting are
    /// two ways to move a permille and there is one bound between them, raised to the grant floor
    /// because a reserve under the smallest grantable share is not a reserve.
    pub fn base_class_reserve_permille(&self) -> u16 {
        self.min_base_class_share_permille.max(self.min_grantable_share_permille())
    }
...
pub enum PalwClassStatusV2 {
    ...
    /// **ADR-0056 Decision 5: reclaimed for producing nothing.**
    ///
...
pub struct PalwClassStateV2 {
    ...
    pub registered_daa: u64,
    /// **Whose bond paid for this class to exist** (ADR-0056 Decision 3), or `None` for a class
...
///
/// One consecutive-epoch counter and the epoch that last folded it. Consecutive is the whole point
/// — a class that misses one epoch has had a bad epoch, and a rule that reclaimed on that would
/// take the seat of anyone whose node restarted.
///
/// It carried a `raise_streak` and a `decay_streak` too, for a walk this line wrote and the merge
/// did not keep (see `with_class_economy_v1`). They are gone rather than left at zero: an unwritten
/// counter inside the state root is a field every node must agree about and nothing can ever
/// disagree over, which is the shape a later reader mistakes for a live signal.
...
/// **The fraction SA-1 prices an accusation at**: `⌈claim.reserved / seat_count⌉`.
///
/// A fraction of the producer's own stake rather than a new constant, so the price scales with the
/// claim being attacked instead of with a number somebody has to pick.
///
/// **This is not by itself what an accusation costs** — see `palw_da_accusation_exposure_v2`, which
/// is what the fold reserves and what `slash_seat` collects. The doc here used to say a reservation
/// of this size meant "ten accusations cannot be funded out of one fee float"; nothing enforced
/// that, because reserving does not touch collateral and the only exposure ceiling in the tree is
/// applied to bonds that want to PRODUCE. The ceiling is now applied at the accusation too, and the
/// figure it counts is `v2`'s capped one, so the sentence is true of the pair and was never true of
/// this function alone.
///
...
/// **What an accusation actually puts at risk** (SA-4 as amended): the fraction above, capped at
/// the same `min_collateral_sompi` the CHARGE is capped at.
///
/// `slash_seat` takes `min(amount, min_collateral_sompi)`, for the reason its own doc gives — the
/// factors of `claim.reserved` are chosen by whoever registered the CLASS, so an uncapped charge
/// would let a registrant make accusing its own class ruinous. `v1` reserved the UNCAPPED fraction
/// while the charge was capped, and the gap is not cosmetic: for any claim where
/// `reserved / seat_count > min_collateral_sompi` — every large claim — the ledger said the accuser
/// had risked a sum the fold could never take from it, so the exposure ceiling below counted
/// liability that did not exist and the sizing argument in `palw_da_accusation_exposure_v1`'s doc
/// ("griefing costs the griefer more per attempt than it costs the producer") described a number
/// nobody ever paid.
///
/// Reserving exactly what can be taken is what makes the ceiling a real bound: a bond's total DA
/// liability is then its exposure ceiling, and no arrangement of claim sizes moves it. Capping the
/// reservation admits MORE concurrent accusations for a big claim than `v1` did (`ceiling / floor`
/// rather than `ceiling / fraction`) and the total charge is the same `ceiling` either way — which
/// is the point: the bound is on the money, not on the count.
pub fn palw_da_accusation_exposure_v2(reserved: u128, seat_count: usize, min_collateral_sompi: u64) -> u128 {
    palw_da_accusation_exposure_v1(reserved, seat_count).min(min_collateral_sompi as u128)
}

/// **The phase a claim would be in if no data-availability accusation were open on it.**
///
/// A DA accusation and the arithmetic fraud court answer different questions — "is the material
/// there" and "is the arithmetic right" — and a producer can be innocent of one and guilty of the
/// other. Until this existed, `validate_court_opened_v2` read `claim.phase` directly, so an open
/// accusation made the claim un-`ReceiptLicensed` and therefore un-challengeable for the whole
/// remaining challenge window. That is not a side effect, it is a purchase: a producer whose
/// ARITHMETIC is fraudulent has any bond accuse it (its own second bond will do — nothing makes
/// the two accounts strangers), answers the single index honestly out of a complete trace, and the
/// challenge window lapses underneath the session. `CourtFraud` would have taken `claim.reserved`
/// and voided the escrow; the accusation costs `min_collateral_sompi`, once.
///
/// So the challenge surface is read THROUGH the disputed phase. `resumed` is exactly the phase the
/// accusation found, `assert_internal_consistency` proves it is one of the three live ones, and on
/// a claim that is not disputed this is the identity — which is why arming nothing changes nothing.
pub fn palw_challenge_surface_phase_v2(phase: &PalwClaimPhaseV2) -> &PalwClaimPhaseV2 {
    match phase {
        PalwClaimPhaseV2::DefaultDisputed { resumed, .. } => resumed,
        other => other,
    }
}
...
// ---------------------------------------------------------------------------------------------
// ADR-0075 security amendment SA-1 / SA-2: what the permissionless lane pays
// ---------------------------------------------------------------------------------------------

/// The mempool's own relay rate, in sompi per kilogram of transaction mass.
///
/// Mirrored rather than imported: the one spelling of this rate that already exists
/// (`kaspa_pq_validator_core::relay_fee_for_compute_mass`, which is what `misaka palw
/// submit-object` sizes every certification carrier with) lives in a crate that DEPENDS on this
/// one, so importing it would be a cycle. `the_rent_rate_is_the_relay_rate_submit_object_pays`
/// pins the arithmetic against the numbers that function is built from, so the two cannot drift
/// silently — the failure mode a mirrored constant has, and the reason this note exists.
const PALW_RENT_RELAY_FEE_SOMPI_PER_KG: u64 = 10_000;

/// The flat floor under [`palw_relay_fee_for_mass_v1`] — `ATTESTATION_TX_FEE_FLOOR_SOMPI`.
const PALW_RENT_FEE_FLOOR_SOMPI: u64 = 250_000;

/// `relay_fee_for_compute_mass`, re-expressed here (see [`PALW_RENT_RELAY_FEE_SOMPI_PER_KG`]).
///
/// Rate × mass, plus a quarter for the margin the submitter tool already adds, floored at the
/// attestation floor. Saturating throughout: this prices a `usize` a peer chose, and a rent rule
/// that could panic on a hostile number would be the denial of service it exists to stop.
pub fn palw_relay_fee_for_mass_v1(mass: u64) -> u64 {
    let min_fee = mass.saturating_mul(PALW_RENT_RELAY_FEE_SOMPI_PER_KG) / 1_000;
    min_fee.saturating_add(min_fee / 4).max(PALW_RENT_FEE_FLOOR_SOMPI)
}

/// The transaction mass one fault vector's grading is priced at (ADR-0075 SA-2).
///
/// A `FamilyCertified` costs every validator one court re-execution per vector, up to
/// [`PALW_CERTIFICATION_MAX_VECTORS`], and the vector count is a field the submitter writes —
/// nothing ties it to how large the object is. So the price is taken from the count rather than
/// from the bytes: a five-kilobyte object carrying thirty-two vectors buys thirty-two
/// re-executions at a five-kilobyte object's price, which is the whole hole.
///
/// The number is the carriage this evidence is ALLOWED to occupy divided by the vectors it is
/// allowed to carry: `PALW_OBJECT_CHUNK_MAX_BYTES × PALW_OBJECT_CHUNK_MAX_COUNT` (800,000, the
/// most a chunked object may span) over `PALW_CERTIFICATION_MAX_VECTORS` (32). A drill that
/// actually uses its vector budget therefore pays what its full-size carriage would have cost at
/// the relay rate, and one that plants six pays for six.
pub const PALW_CERTIFICATION_MASS_PER_VECTOR: u64 =
    (PALW_OBJECT_CHUNK_MAX_BYTES as u64 * PALW_OBJECT_CHUNK_MAX_COUNT as u64) / PALW_CERTIFICATION_MAX_VECTORS as u64;

/// **ADR-0075 SA-2: what a `FamilyCertified` carrying `vectors` fault vectors must have paid.**
///
/// Read at the acceptance filter, before the object is graded and before it consumes one of the
/// block's [`PALW_CERTIFICATION_MAX_PER_BLOCK`] grading slots, so the CPU a block can ask of every
/// validator is bounded by fees its carriers actually paid.
pub fn palw_certification_min_fee_v1(vectors: usize) -> u64 {
...
/// both into every state root (`state_root`) and into `consensus_params_id`, so a bumped version
/// re-mints every V2 network whether or not the fence is armed. That is exactly what the fence
/// discipline exists to prevent, so the deposit cannot be added by an activation; it belongs to
...
/// **Which object kinds ADR-0075 D14's chunk group may assemble into — the ONE spelling.**
///
/// # The gap this names, and why it is closed by a NEW path rather than by widening this one
///
/// A `CourtClosed` cut with [`palw_object_chunks_v1`] pays every carrier's fee, holds one of the
/// eight [`PALW_OBJECT_CHUNK_MAX_GROUPS`] slots for its whole [`PALW_OBJECT_CHUNK_TTL_DAA`], and
/// then fails in the block that completes it — dormant only while no close needed splitting, and
/// W3's 2,250,000-byte ceiling is what arms it.
///
/// **Admitting `CourtClosed` here is the fix this deliberately does not make.** It would put the
/// court's carriage in the certification lane's table, whose `TooManyPendingChunkGroups` is a
/// delay for a drill and a DISPUTE LOST for a court: the mover's rung clock runs while it cannot
/// open, so anyone able to hold eight slots — 3.2 MSK of rent, for the whole TTL — could defeat
/// every prosecution on the network. Raising the count instead would re-price
/// [`PALW_OBJECT_CHUNK_SLOT_MASS`], [`PALW_CERTIFICATION_MASS_PER_VECTOR`] and the SA-1 slot rent,
/// which bound a different lane.
///
/// So the close gets its OWN group, keyed to one of the two bonds the session already binds and
/// therefore unsquattable: [`PalwConsensusObjectV2::CourtCloseDeclared`] and
/// [`PalwConsensusObjectV2::CourtCloseChunk`]. This predicate stays the certification lane's, and
...
// ---------------------------------------------------------------------------------------------
// ADR-0080 design A: the split court close
// ---------------------------------------------------------------------------------------------

/// **Which of the two bonds a dispute binds is moving** (ADR-0080 design A).
///
/// A close group is keyed by `(session_id, side)`, and the side is the reason the court does not
/// share ADR-0075 D14's `pending_chunks`: that table's group id is a free digest, so its rows are
/// mintable in unbounded quantity and its `PALW_OBJECT_CHUNK_MAX_GROUPS` refusal is a delay for a
/// certification and a DISPUTE LOST for a court — the mover's rung clock runs while it cannot
/// open. Here a row exists only for one of the two bonds the session id already names, so there
/// is nothing to squat and no cap to exhaust.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, borsh::BorshSerialize, borsh::BorshDeserialize)]
pub enum PalwCourtSideV1 {
    /// The bond that opened the session — `PalwCourtSessionStateV2::challenger_bond`.
    Challenger,
    /// The bond that produced the disputed claim — `PalwClaimStateV2::bond`.
    Executor,
}

impl PalwCourtSideV1 {
    /// The ladder party this side plays, so a routing decision can be made in one place rather
    /// than re-derived from a session at every arm that needs it.
    pub fn party(self) -> crate::palw_bisect::PalwBisectPartyV1 {
        match self {
            PalwCourtSideV1::Challenger => crate::palw_bisect::PalwBisectPartyV1::Challenger,
            PalwCourtSideV1::Executor => crate::palw_bisect::PalwBisectPartyV1::Responder,
        }
    }
}

/// **The most chunks one close may be declared in, structurally.**
///
/// This is not the ruleset's ceiling — that is `PalwCourtParamsV2::max_close_chunks()`, 27 on the
/// 5f bundle, and it is compared where the bundle is in hand. This is what the ROW can address:
/// `present` is a `u64` bitmap, so a count above 64 could not be represented at all, and 32 is the
/// design's own headroom over the ruleset number. A transition that trusted the ruleset alone
/// would be a transition whose refusal lives in another layer.
pub const PALW_COURT_CLOSE_MAX_CHUNKS: u8 = 32;

/// One chunk of a close is one carrier, so it is bounded by exactly what one carrier holds — the
/// same [`PALW_OBJECT_CHUNK_MAX_BYTES`] the certification lane rides at, which the ADR-0080
/// measurement found is already the largest round number that relays (120,000 standard bytes less
/// the worst-case carrier and the header).
pub const PALW_COURT_CLOSE_CHUNK_MAX_BYTES: usize = PALW_OBJECT_CHUNK_MAX_BYTES;

/// **Blocks budgeted per declared chunk** (ADR-0080 design A §2.5). One block carries one chunk —
/// a block's whole transaction body is `max_block_mass / TRANSIENT_BYTE_TO_MASS_FACTOR` = 125,000
/// bytes and a chunk carrier is ~107,500 — so the margin is "one block per chunk plus three
/// misses": the mover may lose three of every four blocks to other traffic and still assemble.
pub const PALW_COURT_CLOSE_INCLUSION_MARGIN: u64 = 4;

/// **How many close groups one block may complete and adjudicate** (ADR-0080 design A, W9).
///
/// Mirrors [`PALW_CERTIFICATION_MAX_PER_BLOCK`] and exists for the same reason, at the moment
/// that reason stops being automatic: while a close was one transaction, "close bytes in this
...
pub fn palw_close_assembly_daa_v1(count: u8) -> u64 {
    PALW_COURT_CLOSE_INCLUSION_MARGIN.saturating_mul(count as u64)
}

/// The digest a declaration pins one chunk's bytes by — checked at ARRIVAL, so a substituted
/// chunk is refused by the block that carries it rather than discovered at assembly.
pub fn palw_court_close_chunk_digest_v1(bytes: &[u8]) -> Hash64 {
    let mut state = keyed(PALW_STATE_V2_DOMAIN_COURT_CLOSE_CHUNK);
...
///
/// The court lane's analogue of `palw_v2_graded_vector_count`, and it exists for the same reason,
/// at the moment that reason stops being automatic: while a close was one transaction, "close
/// bytes in this block" and "court CPU in this block" were the SAME number and the block's own
/// byte limit bounded both. A split close breaks that coupling — the bytes were spent in earlier
/// blocks and the completing carrier is a few hundred bytes — so the per-block slot
/// ([`PALW_COURT_CLOSE_MAX_PER_BLOCK`]) is counted on this predicate rather than on the bytes.
///
/// It lives HERE rather than in the acceptance filter that reads it, so the filter and the
/// transition cannot hold two answers to one question: every refusal below is one the
/// `CourtCloseChunk` arm makes, in the same order, so nothing the transition rejects on its face
/// can burn the block's slot — the exact hole a one-byte tail opened in the certification lane's
/// cap before `palw_v2_graded_vector_count` replaced "completes a group" there.
///
/// **W7 tightens it.** Once the completing transition assembles, checks `close_digest`, decodes a
/// `PalwCourtVerdictProofV2` and adjudicates, this gains "and the assembled bytes adjudicate".
pub fn palw_court_close_completes_a_group_v1(state: &PalwChainStateV2, object: &PalwConsensusObjectV2) -> bool {
    let PalwConsensusObjectV2::CourtCloseChunk { session_id, side, index, bytes } = object else {
        return false;
    };
    let Some(group) = state.court_close_group(session_id, *side) else {
        return false;
    };
    if *index >= group.count || group.has(*index) {
        return false;
    }
    if bytes.is_empty() || bytes.len() > PALW_COURT_CLOSE_CHUNK_MAX_BYTES {
        return false;
    }
    if palw_court_close_chunk_digest_v1(bytes) != group.chunk_digests[*index as usize] {
        return false;
    }
    group.present | (1u64 << index) == PalwCourtCloseGroupV2::full_mask(group.count)
}

/// **A declared close, mid-assembly** (ADR-0080 design A §2.2), keyed `(session_id, side)`.
///
/// # The one field the design's field list does not have, and why it is here
///
/// The design specified `{ declarer, count, chunk_digests, close_digest, verdict, declared_daa,
/// assembly_deadline_daa, present, deposit_outpoint, deposit }` — digests, never bytes — and put
/// the bytes in a node-local, content-addressed side store re-derivable from block bodies. That
/// buys back a large state root, and it is not available here: a side store makes the COMPLETING
/// transition a function of which blocks a node happens to hold, which is a state-root split
/// between an archival node and a pruned one, and `PalwStateCarriageV2` is exactly the mechanism
/// that exists so a pruned node continues where an archival node stands. So `chunks` holds the
/// bytes, in rooted state, carried like `pending_chunks` — and `present` is kept beside it as the
/// design named it, cross-checked against the keys of `chunks` by
/// [`PalwChainStateV2::assert_internal_consistency`] so the two can never say different things.
///
...
pub struct PalwCourtCloseGroupV2 {
    ...
    /// declaration, so a group can never name a third party.
    pub declarer: PalwBondKeyV2,
    pub count: u8,
    /// Exactly `count` digests, index-ordered: what each chunk's bytes must hash to.
    pub chunk_digests: Vec<Hash64>,
    /// The keyed hash of the concatenation — the belt to the per-chunk braces, so a declaration
    /// that pins consistent parts of an inconsistent whole is still refused at completion.
    pub close_digest: Hash64,
    /// What the mover says its evidence proves. Declared, and therefore checkable against what
    /// the assembled proof actually adjudicates (W7).
    pub verdict: PalwCourtVerdictV2,
    pub declared_daa: u64,
    /// `declared_daa + palw_close_assembly_daa_v1(count)`. **Never above the session's backstop**
    /// — the declaration gate refuses a group that could not finish inside it, and nothing here
    /// ever advances `PalwCourtSessionStateV2::deadline_daa`.
    pub assembly_deadline_daa: u64,
    /// One bit per index; `count <= PALW_COURT_CLOSE_MAX_CHUNKS` so a `u64` is generous.
    pub present: u64,
    /// The outpoint backing [`Self::deposit`] — the field ADR-0075 SA-1 named as the reason its
    /// own deposit could not be built ("the chain to REMEMBER which outpoint backs which group").
    /// Written here; the lock and the burn are W10's.
    pub deposit_outpoint: TransactionOutpoint,
    pub deposit: u64,
    /// The chunk bytes that have arrived, by index. See the type doc for why they are here.
    pub chunks: BTreeMap<u8, Vec<u8>>,
}

impl PalwCourtCloseGroupV2 {
    /// The bitmap of a complete group of `count` chunks.
    pub fn full_mask(count: u8) -> u64 {
        if count >= 64 { u64::MAX } else { (1u64 << count) - 1 }
    }

    pub fn has(&self, index: u8) -> bool {
        index < 64 && self.present & (1u64 << index) != 0
    }

    pub fn is_complete(&self) -> bool {
        self.present == Self::full_mask(self.count)
    }
}
...
pub enum PalwStateV2Error {
    ...
    // ADR-0080 design A: the split court close.
    #[error("session {session} holds no declared close on this side")]
    MissingCourtCloseGroup { session: Hash64 },
    #[error("session {session} already holds a declared close on this side; one declaration per side, ever")]
    DuplicateCourtCloseGroup { session: Hash64 },
    #[error("a close may be declared in 1..={max} chunks, not {count}")]
    CourtCloseCountOutOfRange { count: u8, max: u8 },
    #[error("the declaration for session {session} does not carry exactly one digest per declared chunk")]
    CourtCloseDigestsIncoherent { session: Hash64 },
    #[error("chunk {index} is outside a close declared in {count} chunks")]
    CourtCloseIndexOutOfRange { index: u8, count: u8 },
    #[error("the declared close of session {session} already holds chunk {index}")]
    CourtCloseChunkDuplicate { session: Hash64, index: u8 },
    #[error("a close chunk carries {bytes} bytes, more than the {max} one carrier holds")]
    CourtCloseChunkTooLarge { bytes: usize, max: usize },
    #[error("chunk {index} of session {session} is not the bytes the declaration pinned at that index")]
    CourtCloseChunkDigestMismatch { session: Hash64, index: u8 },
    #[error("the declared side is not a bond session {session} binds")]
    CourtCloseSideNotBound { session: Hash64 },
    #[error(
        "a close declared in {count} chunks needs until {needed} to assemble and session {session}'s backstop is {deadline} — a \
         declaration may never extend the backstop, so one that cannot finish inside it is refused at the rung it would suspend"
    )]
    ...
    #[error("a derivation names claim {0}, which this chain does not hold")]
    DerivedClaimMissing(Hash64),
    #[error("a derivation names claim {0}, which is not a free-prompt claim — only a person's prompt has an answer to derive from")]
    DerivedClaimIsNotFreePrompt(Hash64),
    #[error("a derivation of claim {claim} declares output root {declared}; the claim committed {committed}")]
    DerivedOutputRootMismatch { claim: Hash64, declared: Hash64, committed: Hash64 },
    #[error("a derivation of claim {claim} is signed by a key that is not the claim's executor bond key")]
    DerivedSignerIsNotTheExecutor { claim: Hash64 },
    #[error("claim {claim} already carries a derivation under transformer {transformer}")]
    DuplicateDerivation { claim: Hash64, transformer: Hash64 },
    #[error("claim {claim} already carries {max} derivations, the most one claim may hold (ADR-0078 Decision 4)")]
    TooManyDerivations { claim: Hash64, max: usize },
    // ---- ADR-0062 as amended: the data-availability court ----
    #[error(
...
impl PalwChainStateV2 {
    ...
    pub fn assert_internal_consistency_v2(
        ...
        //
        // **And a `Dormant` one holds none either** (ADR-0056 Decision 5). It is the same shape as
        ...
        // **ADR-0080 design A: a close group is a row ABOUT a session, and it says three things
        // twice.** `present` and the keys of `chunks` are the same fact in two encodings — the
        // design named the bitmap and the carriage has to hold the bytes — and a carriage is a
        // state somebody else wrote, so the two are compared here rather than believed. So is the
        // session: a group whose session the state does not hold is a row nothing will ever sweep,
        // which is exactly the orphan class the derived table above pays for.
        for ((session, side), group) in &self.court_close_groups {
            if !self.court_sessions.contains_key(session) {
                return Err(PalwStateV2Error::CarriageInconsistent(format!(
                    "a close group names session {session}, which the state does not hold — the row would never be swept"
                )));
            }
            if group.count == 0 || group.count > PALW_COURT_CLOSE_MAX_CHUNKS {
                return Err(PalwStateV2Error::CarriageInconsistent(format!(
                    "the close group of session {session} declares {} chunks, outside 1..={PALW_COURT_CLOSE_MAX_CHUNKS}",
                    group.count
                )));
            }
            if group.chunk_digests.len() != group.count as usize {
                return Err(PalwStateV2Error::CarriageInconsistent(format!(
                    "the close group of session {session} declares {} chunks and pins {} digests",
                    group.count,
                    group.chunk_digests.len()
                )));
            }
            let mut mask = 0u64;
            for (index, bytes) in &group.chunks {
                if *index >= group.count {
                    return Err(PalwStateV2Error::CarriageInconsistent(format!(
                        "the close group of session {session} holds chunk {index} of a {}-chunk close",
                        group.count
                    )));
                }
                if bytes.is_empty() || bytes.len() > PALW_COURT_CLOSE_CHUNK_MAX_BYTES {
                    return Err(PalwStateV2Error::CarriageInconsistent(format!(
                        "chunk {index} of session {session}'s close carries {} bytes",
                        bytes.len()
                    )));
                }
                if palw_court_close_chunk_digest_v1(bytes) != group.chunk_digests[*index as usize] {
                    return Err(PalwStateV2Error::CarriageInconsistent(format!(
                        "chunk {index} of session {session}'s close is not the bytes its declaration pinned"
                    )));
                }
                mask |= 1u64 << index;
            }
            if mask != group.present {
                return Err(PalwStateV2Error::CarriageInconsistent(format!(
                    "the close group of session {session} claims presence {:#x} and holds {mask:#x}",
                    group.present
                )));
            }
            // The declaration's own gate, re-asked of the row: it may never sit past the backstop,
            // because a declaration that could not finish inside it is a rung suspended for free.
            let backstop = self.court_sessions[session].deadline_daa;
            if group.assembly_deadline_daa > backstop {
                return Err(PalwStateV2Error::CarriageInconsistent(format!(
                    "the close group of session {session} assembles until {} and the backstop is {backstop}",
                    group.assembly_deadline_daa
                )));
            }
            let _ = side;
        }
...
pub enum PalwDeltaEntryV2 {
    Bond {
        key: PalwBondKeyV2,
        old: Option<PalwBondStateV2>,
        new: Option<PalwBondStateV2>,
    },
    Exposure {
        key: PalwBondKeyV2,
        old: Option<u128>,
    ...
    /// ADR-0056 Decisions 3 and 5. Journalled like every other write so a delta replays exactly.
    ...
    ClassWalk {
        key: Hash64,
        old: Option<PalwClassWalkV2>,
        new: Option<PalwClassWalkV2>,
    },
    Class {
        key: Hash64,
        old: Option<PalwClassStateV2>,
        new: Option<PalwClassStateV2>,
    },
    Target {
        key: Hash64,
        old: Option<PalwClassTargetV2>,
        new: Option<PalwClassTargetV2>,
    },
    Share {
        key: Hash64,
        old: Option<u16>,
    ...
    EpochBudgets {
        old: Option<PalwEpochBudgetsV2>,
        new: Option<PalwEpochBudgetsV2>,
    },
    ReceiptTarget {
        key: Hash64,
        old: Option<PalwClassTargetV2>,
    ...
    Capability {
        key: Hash64,
        old: Option<PalwCapabilityStateV2>,
        new: Option<PalwCapabilityStateV2>,
    },
    Claim {
        key: Hash64,
        old: Option<PalwClaimStateV2>,
        new: Option<PalwClaimStateV2>,
    },
    Payout {
        key: Hash64,
    ...
    Panel {
        key: Hash64,
        old: Option<PalwPanelStateV2>,
        new: Option<PalwPanelStateV2>,
    },
    Court {
        key: Hash64,
        old: Option<PalwCourtSessionStateV2>,
        new: Option<PalwCourtSessionStateV2>,
    },
    Epoch {
        key: Hash64,
        old: Option<PalwEpochCounterV2>,
        new: Option<PalwEpochCounterV2>,
    },
    ReceiptEpoch {
        key: Hash64,
        old: Option<PalwEpochCounterV2>,
    ...
    /// ADR-0080 design A. Journalled like every other write, which is what makes a reorg of the
    /// block that carried chunk 3 of 8 revert the bit and the deadline by the delta alone.
    CourtCloseGroup {
        key: (Hash64, PalwCourtSideV1),
        old: Option<PalwCourtCloseGroupV2>,
        new: Option<PalwCourtCloseGroupV2>,
    },
    /// ADR-0078 Decision 4. Journalled like every other write so a delta replays exactly.
    DerivedArtifact {
...
impl<'a> TransitionBuilder<'a> {
    ...
    fn retire_claim(&mut self, id: Hash64, claim: &PalwClaimStateV2) -> Result<(), PalwStateV2Error> {
        ...
        //
        // **The 2026-09-02 audit found the same defect independently, and its reachability
        // measurement is recorded here because it is what sizes the dormant exposure below.**
        // `assert_internal_consistency` recomputes a `Final` free-prompt claim's contribution as
        // `per_quantum × |spent|`, so an attempt-only fold subtracts that from the recomputation
        // and adds it back to nothing. The check runs inside `into_state`, which `load_tip` calls
        // on every virtual resolution and whose caller panics — so the state is written by every
        // node and then kills every node, on the next resolution and on every restart. The shipped
        // ruleset retires at 3,000 DAA and the receipt use window is 600, so a licensed claim with
        // one spent quantum reaches it in the ordinary course. That audit's remedy was to fold both
...
/// D4's verdict policy** — ADR-0071's capability bound, ADR-0069 Decision 7's weight rule, and ADR-0062's DA court.
///
/// Both arrived as separate faces of the same shape and are one face here on purpose: this is what
/// the object-acceptance REHEARSAL, the genesis fold and the state-sync walk call, and each exists
/// to predict what [`apply_palw_transition_v6`] will compute. A rehearsal that resolved the
/// capability fence differently from the fold admits a declaration the fold then refuses, taking
/// the whole block down for a reason acceptance said was fine; a rehearsal that priced a weightless
/// class's work differently computes a different state root for the same block. Three lanes wrote three faces of this
/// shape in one batch and a fourth would have been written next; separate faces let a policy be
/// added to one and forgotten in the other, which is the whole defect class. All-`false` is
/// byte-identical to the transition before any of the parameters existed.
...
pub fn apply_palw_transition_v4(
    ...
    apply_palw_transition_v6(parent, params, admission, ctx, accepted_objects, block_work, merged_work, false, false, false, false)
}
...
/// [`apply_palw_transition_v5`] with **ADR-0071's capability bound** (SA-1..SA-4) and **ADR-0069 Decision 7**.
///
...
///
/// **And with ADR-0069 Decision 7's weight rule.** `uncertified_weightless` is `Params::palw_uncertified_weightless` resolved at `ctx.daa_score`,
...
/// block is otherwise untouched, which is the whole of Decision 7. A fold that disagrees with its
/// peers about what a weightless class's block is worth computes a different state root, so the
/// same "every production caller must reach this one" applies to it.
///
/// **And with ADR-0062's fence.** `da_court` is `Params::palw_da_court` resolved at
/// `ctx.daa_score`. `false` is every shipped preset and is byte-identical to the transition before
/// that ADR: both DA object arms refuse outright, and no claim can be in `DefaultDisputed` because
/// nothing could ever have put it there.
#[allow(clippy::too_many_arguments)]
pub fn apply_palw_transition_v6(
    ...
    // 2d. Reclamation (this line's ADR Decision 5), at the same boundary and off the same census.
    //     AFTER the growth step, because a class the growth rule just decayed to nothing and a
...
/// **Advance a refuted claim's anchor by the length of the session it just answered** (ADR-0062
/// SA-7).
///
/// The phase's anchor IS the clock — every deadline in this ruleset is `anchor + window` — so
/// pausing the clock for a session is advancing the anchor by the session's length, and nothing
/// else in the fold has to learn a new concept. In particular `assert_deadline_consistency` and
/// `rebuild_deadline_index_v2` keep deriving the deadline from the record alone, which is what lets
/// the deadline index stay out of the state root.
///
/// **Why it must extend and not merely resume.** `W_disclose = window_challenge`, and every
/// ConsensusV2 preset has `window_challenge > window_receipt` and `> window_bind` (the shipped
/// reference bundle: bind 600, receipt 600, challenge 1200). A claim that is accused while
/// `PanelBound` and answers HONESTLY on the last block of its disclose window therefore came back
/// to a receipt deadline 600 DAA in the past, and the very next sweep redrew it; the redraw leaves
/// the old panel record in place, so a second accusation was admissible immediately, and the second
/// honest answer came back to an expired BIND deadline and the sweep voided the claim. Two
/// accusations, `2 × min_collateral_sompi`, and an honest producer that answered both correctly and
/// on time lost its escrow — audit3 H4's shape against a producer that did everything right, and a
/// responder cannot help because answering is what triggers it.
///
/// **Which anchors move, and which one deliberately does not.** The pause returns time the session
/// TOOK FROM the producer, and it takes time only where the producer owed an object it could not
/// deliver: a disputed claim is neither `Provisional` nor `PanelBound`, so the `PanelBound` object
/// arm and the `ReceiptLicensed` object arm both refuse it, and every DAA of the session is a DAA
/// the producer was locked out of its own window. `ReceiptLicensed` names no such obligation — the
/// producer has finished and is only waiting — so it resumes UNMOVED, and the claim finalizes at
/// the first sweep after the disclosure. Advancing it too would only delay an honest producer's
/// `Final` further, and it is the delay, not the deadline, that a serial accuser is buying.
///
/// Nothing else may move. `accepted_daa` anchors `trace_retention_daa` and the accuse window, so
/// advancing it would extend the producer's own retention obligation as a reward for being accused;
/// the extension therefore rides `rebound_daa` for a `Provisional` claim, which is the anchor
/// `bind_base_daa` already reads. That claim is necessarily a REDRAWN one — the accusation arm
/// requires a bound panel, and the only way to hold a panel while `Provisional` is to have been
/// redrawn — so `rebound_daa` is already `Some` and this consumes no redraw. The accusation arm
/// refuses the impossible case outright rather than leaving the invariant implicit.
fn resume_claim_after_da_session_v2(
    claim_id: Hash64,
    claim: &PalwClaimStateV2,
    resumed: PalwClaimPhaseV2,
    paused_daa: u64,
) -> Result<PalwClaimStateV2, PalwStateV2Error> {
    let mut restored = claim.clone();
    let shift = |anchor: u64| anchor.checked_add(paused_daa).ok_or(PalwStateV2Error::Overflow("da resume anchor"));
    match resumed {
        PalwClaimPhaseV2::Provisional => {
            restored.rebound_daa = Some(shift(claim.bind_base_daa())?);
            restored.phase = PalwClaimPhaseV2::Provisional;
        }
        PalwClaimPhaseV2::PanelBound { bound_daa } => {
            restored.phase = PalwClaimPhaseV2::PanelBound { bound_daa: shift(bound_daa)? };
        }
        PalwClaimPhaseV2::ReceiptLicensed { licensed_daa } => {
            restored.phase = PalwClaimPhaseV2::ReceiptLicensed { licensed_daa };
        }
        // Unreachable by construction and stated as an error for the reason
        // `rearm_claim_after_da_session` states it: the accusation arm refuses a terminal claim and
        // refuses a disputed one, and `assert_internal_consistency` refuses a loaded state that
        // says otherwise — but a fold every node runs on peer input does not get to be sure.
...
///
/// The restored deadline is derived from the anchor `resume_claim_after_da_session_v2` has already
/// advanced by the session's length, so it is never in this block's past for a claim that answered
/// on time. What it may NOT do is RESTART, which is what D4's "return to `ReceiptLicensed
/// { licensed_daa: the disclosure's DAA }`" would have done — see the phase's own note on why
/// serial accusations would then hold a claim open forever. Advancing by exactly the elapsed
/// session is the difference between the two: a producer gets back the time it had left and not one
/// DAA more.
...
fn sweep_court_deadlines(builder: &mut TransitionBuilder<'_>, ctx: &PalwBlockContextV2) -> Result<(), PalwStateV2Error> {
    while let Some(&(deadline, session_id)) = builder.state.court_deadlines.iter().next() {
        ...
        // **ADR-0080 design A: a failed declaration loses on its OWN side.**
        //
        // A close is legal only at `Terminal`, where the rung clock is off and the backstop is the
        // only thing that fires — and the backstop closes CHALLENGER-side. So an EXECUTOR that
        // declares a close and then does not assemble it would end the session against its own
        // prosecutor: it could suspend nothing, spend nothing, and win. That is not a residual, it
        // is the mechanism inverted into a weapon, and it is the one thing the declaration must
        // not make possible.
        //
        // So a live, INCOMPLETE group decides the direction: the declarer defaulted on a close it
        // announced and pinned every byte of, which is the `Responder` treatment the rung sweep
        // already gives an executor that will not stand behind its own root. A CHALLENGER that
        // fails the same way lands on the challenger-side close it would have landed on anyway.
        //
        // **A COMPLETE group is deliberately not convicted here.** Assembling and adjudicating it
        // is W7; until that lands, a group that has all its bytes has done everything this branch
        // asks of it, and the sweep falls through to exactly the behaviour that shipped. The arm
        // decides whether to DESTROY collateral, and the direction to be wrong in is the one that
        // does not.
        ...
        builder.write_court(session_id, None);
        match failed_declarer_side {
            Some(PalwCourtSideV1::Executor) => {
                if !claim.phase.is_terminal() {
                    builder.void_and_slash(session.claim, &claim, ctx.daa_score, PalwVoidReasonV2::CourtFraud)?;
                }
            }
            // The backstop's own direction, and a challenger's failed declaration is the same
            // unproven accusation it was before the declaration existed.
            _ => rearm_after_challenger_side_close(builder, ctx, session.claim, &claim, session.challenger_bond)?,
        }
    }
    Ok(())
}
...
/// **Reclamation: a class that has stopped producing gives its seat back** (this line's ADR
/// Decision 5).
///
/// Runs at the same boundary `apply_class_retargets` and `apply_class_share_growth` run at, off
/// the same closed epoch's counters, and BEFORE `ensure_epoch_budgets` derives the new epoch's
/// budgets — a share this returns is a share those budgets must be derived from.
///
/// # Why this is not the growth rule's decay arm
///
/// ADR-0054 Decision 1 moves ONE step of permille per epoch off filled-budget, which is a price
/// signal: a class running below its share is running at its natural rate and its share walks down
/// toward it. That never removes a class, by design — Decision 2 stops decay at the grant floor,
/// because a share below it is a zero budget and a class that cannot produce is a frozen class
/// wearing the wrong status.
///
/// Reclamation answers a different question: not "is this class using its cadence" but "is this
/// class still here". A registrant that walked away holds a seat, a share, and — Decision 3 — an
/// exposure reservation against its bond, and none of the three come back on their own. So the
/// signal is absolute silence (`produced == 0`) for `reclaim_epochs` CONSECUTIVE closed epochs, and
/// the effect is total: the whole share returns to the incumbents, the record goes `Dormant`, and
/// the reservation is released.
///
/// The registry entry itself is kept. A class's past work stays adjudicable after its operators
/// leave — a court that could not resolve a class id would turn walking away into an amnesty.
fn apply_class_reclamation(
    builder: &mut TransitionBuilder<'_>,
    parent: &PalwChainStateV2,
    ctx: &PalwBlockContextV2,
) -> Result<(), PalwStateV2Error> {
    let params = builder.params;
    if params.reclaim_epochs() == 0 {
        return Ok(()); // Decision 5 is off on this network.
    }
    ...
    for id in &walking {
        ...
        let produced = builder
            ...
            .unwrap_or(0);
        let budget =
            budgets.as_ref().filter(|b| b.epoch_index == closed_epoch).and_then(|b| b.budget_blocks.get(id).copied()).unwrap_or(0);
        // **A class with no budget for the closed epoch is not measured.** It could not have
        // produced under a ceiling nobody gave it — the mid-epoch activation case — and counting
        // that silence would reclaim a class for the boundary's own arithmetic.
        if budget == 0 {
        ...
        walk.last_epoch = closed_epoch;
        if walk.idle_streak >= params.reclaim_epochs() {
            reclaims.push(*id);
...
fn redistribute_permille_v1(
    ...
    let holders: Vec<(Hash64, u32)> =
        builder.state.class_shares.iter().filter(|(id, _)| Some(**id) != exclude).map(|(id, share)| (*id, *share as u32)).collect();
    ...
    let mut shares: Vec<(Hash64, u32, u32)> = holders.iter().map(|(id, s)| (*id, s * amount / total, s * amount % total)).collect();
...
fn apply_object(
    ...
    match object {
        ...
        PalwConsensusObjectV2::ClassRegistered {
            ...
            // the ADR-0056 Decision 3 reservation is referential integrity, the transition's own
        ...
        // from committed onward: a derivation of a claim that later voids is a derivation of a
        // voided claim, and says so when read), the declared output root is the claim's own, the
        // signer is the claim's executor, the `(claim, transformer)` pair is new, and the claim is
        ...
        PalwConsensusObjectV2::DerivedArtifactV1 { object, signature } => {
            ...
            if claim.output_root != object.output_root {
                return Err(PalwStateV2Error::DerivedOutputRootMismatch {
                    claim: object.claim_id,
                    declared: object.output_root,
                    committed: claim.output_root,
                });
            }
            // The same comparison `FreePromptCommitted` makes: the acceptance layer proved the
            // object is signed by the key it declares; only the registry can say whether that key
            // is the claim's bond's.
            let bond_record = builder.state.bonds.get(&claim.bond).ok_or(PalwStateV2Error::MissingBond(claim.bond))?;
            if bond_record.pubkey != object.executor_pubkey {
                return Err(PalwStateV2Error::DerivedSignerIsNotTheExecutor { claim: object.claim_id });
            }
            let key = PalwDerivedKeyV1 { claim: object.claim_id, transformer: object.transformer_id };
            if builder.state.derived_artifacts.contains_key(&key) {
                return Err(PalwStateV2Error::DuplicateDerivation { claim: object.claim_id, transformer: object.transformer_id });
            }
            let held = builder.state.derived_artifacts.range(PalwDerivedKeyV1::claim_range(object.claim_id)).count();
            if held >= PALW_DERIVED_MAX_PER_CLAIM {
                return Err(PalwStateV2Error::TooManyDerivations { claim: object.claim_id, max: PALW_DERIVED_MAX_PER_CLAIM });
            }
            builder.write_derived_artifact(key, Some(PalwDerivedRowV1::from_object(object, ctx.daa_score)));
        }
        ...
        PalwConsensusObjectV2::CourtOpened { session_id, claim: claim_id, challenger_bond, space, space_size, signature: _ } => {
            ...
            //
            // **`DefaultDisputed` is deliberately not in this set** (ADR-0062 SA-7). Past the DA
            // fence a claim can be challengeable while disputed — `validate_court_opened_v2` reads
            // the challenge surface through the accusation — and the deadline such a claim holds is
            // the DISCLOSE deadline, not a path to `Final`. Disarming it would delete the silence
            // clock the accusation runs on, and the DA sweep would never fire: opening a court
            // would then be the way to cancel an accusation. The disclosure re-arms through
            // `rearm_claim_after_da_session`, which asks `open_courts_by_claim` the same question
            // this does, so the two cannot arm a final deadline behind each other's back.
        ...
        PalwConsensusObjectV2::CourtCloseDeclared { session_id, side, count, chunk_digests, close_digest, verdict, signature: _ } => {
            ...
            // **The declarer is READ from the two bonds the session id already binds** — the
            // challenger from the session record, the executor from the claim — so a declaration
            // can never name a third party and the row can never be squatted. Whether the
            // signature is that bond's is the acceptance layer's, where the registry is in hand:
            // the same split `CourtDisclosed` and `CourtVerdictPosted` already use.
            ...
            // free window by re-declaring.
            if builder.state.court_close_groups.contains_key(&(*session_id, *side)) {
                return Err(PalwStateV2Error::DuplicateCourtCloseGroup { session: *session_id });
            }
            if *count == 0 || *count > PALW_COURT_CLOSE_MAX_CHUNKS {
                return Err(PalwStateV2Error::CourtCloseCountOutOfRange { count: *count, max: PALW_COURT_CLOSE_MAX_CHUNKS });
            }
            if chunk_digests.len() != *count as usize {
                return Err(PalwStateV2Error::CourtCloseDigestsIncoherent { session: *session_id });
            }
            // **The declaration's clock is the BACKSTOP, and the backstop never moves.**
            //
            // A close is legal only at `Terminal`, and `court_next_deadline_v2` returns
            // `session.deadline_daa` alone there — "Terminal has no move the responder can make"
            // (audit M2-4). So the only clock that can still end this session is the whole-session
            // backstop, which closes on the CHALLENGER's side. A declaration that cannot assemble
            // inside it is therefore not a slow close, it is a rung suspended for nothing, and
            // extending the backstop to fit it would hand either party a free window for the price
            // of a declaration. Refused here instead, at the block that carries it, where the
            // mover can still make a smaller one.
            let needed = ctx
                .daa_score
                .checked_add(palw_close_assembly_daa_v1(*count))
                .ok_or(PalwStateV2Error::Overflow("close assembly deadline"))?;
            if needed > session.deadline_daa {
                return Err(PalwStateV2Error::CourtCloseCannotAssemble {
                    session: *session_id,
                    count: *count,
                    needed,
                    deadline: session.deadline_daa,
                });
            }
            builder.write_court_close_group(
                ...
                Some(PalwCourtCloseGroupV2 {
                    ...
                    present: 0,
                    // W10 writes the backing outpoint and the amount; the FIELDS are what this
                    // state-version bump exists to afford, and ADR-0075 SA-1's own note names
                    // their absence as the reason its deposit could not be built.
        ...
        PalwConsensusObjectV2::CourtCloseChunk { session_id, side, index, bytes } => {
            let mut group = builder
                ...
                .court_close_groups
                .get(&(*session_id, *side))
                .ok_or(PalwStateV2Error::MissingCourtCloseGroup { session: *session_id })?
                .clone();
            if ctx.daa_score > group.assembly_deadline_daa {
                return Err(PalwStateV2Error::CourtCloseGroupExpired { session: *session_id });
            }
            if *index >= group.count {
                return Err(PalwStateV2Error::CourtCloseIndexOutOfRange { index: *index, count: group.count });
            }
            if group.has(*index) {
                return Err(PalwStateV2Error::CourtCloseChunkDuplicate { session: *session_id, index: *index });
            }
            if bytes.is_empty() || bytes.len() > PALW_COURT_CLOSE_CHUNK_MAX_BYTES {
                return Err(PalwStateV2Error::CourtCloseChunkTooLarge { bytes: bytes.len(), max: PALW_COURT_CLOSE_CHUNK_MAX_BYTES });
            }
            // **Refused at ARRIVAL, not at assembly.** The declaration already pinned this index's
            // bytes, so a chunk is either the pinned preimage or it is nothing — strictly earlier
            // than ADR-0075 D14, where a substituted part is only discovered at
            // `ChunkGroupHashMismatch` after the table has carried it for the whole TTL.
            if palw_court_close_chunk_digest_v1(bytes) != group.chunk_digests[*index as usize] {
                return Err(PalwStateV2Error::CourtCloseChunkDigestMismatch { session: *session_id, index: *index });
            }
            group.present |= 1u64 << index;
            group.chunks.insert(*index, bytes.clone());
            builder.write_court_close_group((*session_id, *side), Some(group));
            // **What does NOT happen here, named rather than implied.** The completing chunk does
            // not assemble, decode, check `close_digest`, run `check_close_cost_v2` /
            // `adjudicate_court_close_v2` or apply the `CourtClosed` state machine: that is W7,
            // and it is the only reason this branch's `close_digest` and `verdict` are written and
...
pub struct PalwStateCarriageV2 {
    ...
    /// ADR-0056 Decisions 3 and 5. Carried like every other accumulator so a syncing node rebuilds
    ...
    /// **ADR-0080 design A: the declared closes, carried with their BYTES.**
    ///
    /// A pruned node that could not complete a group a declaration already pinned would compute
    /// a different state root from an archival one at the completing block, which is the split
    /// this snapshot exists to prevent — the same argument `pending_chunks` is carried under.
    pub court_close_groups: BTreeMap<(Hash64, PalwCourtSideV1), PalwCourtCloseGroupV2>,
    /// ADR-0078 Decision 4: the derivation table, carried because the root hashes it.
    pub derived_artifacts: BTreeMap<crate::palw_derived_v1::PalwDerivedKeyV1, crate::palw_derived_v1::PalwDerivedRowV1>,
...
impl PalwStateCarriageV2 {
    pub fn from_state(state: &PalwChainStateV2) -> Self {
        Self {
            ...
            court_close_groups: state.court_close_groups.clone(),
            derived_artifacts: state.derived_artifacts.clone(),
    ...
    pub fn into_state_v2(
        ...
        let mut state = PalwChainStateV2 {
            ...
            court_close_groups: self.court_close_groups,
...
pub(crate) mod tests {
    ...
    /// Params with the class economy ON: registration costs 1,000 sompi, reclamation takes 3 idle
    /// epochs, the growth step is 250‰ of a class's own share, and the floor is protected at 300‰.
    fn economy_params() -> PalwStateParamsV2 {
        // The floor's reserve is the audit batch's own field now, set by its own builder — one
        // bound, whichever path tries to move a permille (growth, decay, or a grant).
        params()
            .with_class_economy_v1(1_000, 3)
            .unwrap()
            .with_min_base_class_share_permille(300)
            .unwrap()
    ...
    fn a_registration_reserves_its_registrants_collateral_while_the_class_lives() {
        ...
        let bond = bond_key(1);
        let (state, _) =
            apply_palw_transition_v2(&PalwChainStateV2::genesis(), &p, &ctx(1, 10, 1), &[registration(h64(1), 1000, None)], None)
        ...
        let (state, _) = apply_palw_transition_v2(&state, &p, &ctx(2, 11, 2), &[registration(h64(2), 1, Some(bond))], None)
        ...
        let (state, _) = apply_palw_transition_v2(&state, &p, &ctx(3, 12, 3), &[registration(h64(3), 1, Some(bond))], None)
    ...
    fn a_class_that_produces_nothing_is_reclaimed_and_gives_everything_back() {
        ...
        let (back, _) =
            apply_palw_transition_v2(&state, &p, &ctx(99, 10 + 4 * epoch, 99), &[registration(h64(2), 1, Some(bond))], None)
    ...
    /// **ADR-0054 Decision 1 at the transition layer: filling the budget raises the share, and the
    /// table still conserves.**
    ///
    /// The growth rule has its own epoch-table suite (`palw_class_daa_epoch_table`); what this adds
    /// is the seam it shares with reclamation. Both run at the same boundary, off the same census,
    /// against the same share table, and this asserts the pair leaves 1000‰ behind — a raise funded
    /// from the floor and a reclamation returning a whole share are two writers on one collection,
    /// which is where a conservation bug would live. Production is written straight into the epoch
    /// counters, which is what an attempt does.
    #[test]
    fn filling_the_budget_raises_the_share_and_the_table_still_conserves() {
        ...
        assert!(ended > started, "a class that filled its budget must be raised: {started}‰ -> {ended}‰");
        // `max(1‰, share x g / 1000)` per qualifying boundary, and three boundaries qualified — so
        // the entrant cannot have moved further than the rule's own step allows it to.
        assert!(ended - started <= 3, "at most one step per boundary, got {started}‰ -> {ended}‰");
        assert!(
    ...
    fn derived_object(claim_word: u64, transformer: u64, output_root: Hash64, pubkey: Vec<u8>) -> PalwConsensusObjectV2 {
        use crate::palw_derived_v1::{PALW_DERIVED_V1_VERSION, PalwDerivedArtifactV1, kind};
        PalwConsensusObjectV2::DerivedArtifactV1 {
            object: Box::new(PalwDerivedArtifactV1 {
                version: PALW_DERIVED_V1_VERSION,
                network_domain: h64(999),
                claim_id: h64(claim_word),
                output_root,
                grammar_id: h64(0x6A),
                transformer_id: h64(transformer),
                kind: kind::SCENE,
                dsl_hash: h64(0xD5),
                artifact_hash: h64(0xA7),
    ...
    /// The fixture bond registers a 4-byte key (`register_class_and_bond`), so the shape check
    /// — which wants an ML-DSA-87 key — would refuse every derivation before the claim is even
    /// looked up. The transition tests below run with the shape check's key-length rule
    /// satisfied by the fixture bond's own key length, which the state compares byte for byte.
    fn fp_executor_key() -> Vec<u8> {
        vec![7; crate::palw_derived_v1::PALW_DERIVED_V1_EXECUTOR_PUBKEY_LEN]
    }

    /// A free-prompt fixture whose bond registered an ML-DSA-87-sized key, so a derivation's
    /// shape check and the bond comparison can both be exercised.
    fn fp_state_with_claim(p: &PalwStateParamsV2) -> PalwChainStateV2 {
        let genesis = PalwChainStateV2::genesis();
        let mut objects = register_class_and_bond();
        ...
        if let PalwConsensusObjectV2::FreePromptCommitted { executor_pubkey, .. } = &mut commit {
            *executor_pubkey = fp_executor_key();
        }
        let (s2, _) = apply(&s1, p, &ctx(2, 101, 2), &[commit], None);
        assert!(s2.claim(&h64(0xFC)).is_some());
        s2
    }

    /// **ADR-0078 Decision 4 and X5: a derivation is recorded beside its claim, and it credits no
    /// weight, no payment and no exposure.** The state after the object equals the state after
    /// an empty block in every table but the derivation table; the delta replays and reverts.
    #[test]
    fn a_derivation_is_recorded_beside_its_claim_and_credits_nothing() {
        let p = params();
        ...
        let (with, delta) = apply(&s2, &p, &ctx(3, 102, 3), &[object], None);
        let (without, _) = apply(&s2, &p, &ctx(3, 102, 3), &[], None);
        let row = with.derived_artifact(&h64(0xFC), &h64(0x71)).expect("the row is beside the claim");
        assert_eq!(row.kind, crate::palw_derived_v1::kind::SCENE);
        assert_eq!(row.artifact_bytes, 4_096);
        assert_eq!(row.accepted_daa, 102);
        assert_eq!(with.derived_artifacts_of(h64(0xFC)).count(), 1);
        // X5, table by table.
        assert_eq!(with.safe_weight(), without.safe_weight(), "no weight");
        assert_eq!(with.bounded_immature(), without.bounded_immature(), "no immature weight");
        assert_eq!(with.reserved_exposure(&bond_key(1)), without.reserved_exposure(&bond_key(1)), "no exposure");
        assert_eq!(with.pending_payouts_iter().count(), without.pending_payouts_iter().count(), "no payment");
        assert_eq!(with.claim(&h64(0xFC)), without.claim(&h64(0xFC)), "the claim record is untouched");
        assert_ne!(with.state_root(), without.state_root(), "and the root sees the row");
        // The delta is the transition.
        let replayed = apply_delta_v2(&s2, &delta, &p).expect("replays");
        assert_eq!(replayed.state_root(), with.state_root());
        let reverted = revert_delta_v2(&with, &delta, &p).expect("reverts");
        assert_eq!(reverted.state_root(), s2.state_root());
        assert!(delta.entries.iter().any(|e| matches!(e, PalwDeltaEntryV2::DerivedArtifact { .. })), "journalled");
    }

    /// **X2, refusal by refusal**: a missing claim, another claim's output root, a key that is
    /// not the bond's, a duplicate `(claim, transformer)`, a fifth derivation, an attempt claim,
    /// and a shape the ride list would already have refused.
    #[test]
    fn a_derivation_is_refused_by_name_for_every_x2_clause() {
        let p = params();
        let s2 = fp_state_with_claim(&p);
        let output_root = s2.claim(&h64(0xFC)).unwrap().output_root;
        let point = ctx(3, 102, 3);

        let err = apply_palw_transition_v2(&s2, &p, &point, &[derived_object(0xFD, 0x71, output_root, fp_executor_key())], None);
        assert!(matches!(err, Err(PalwStateV2Error::DerivedClaimMissing(c)) if c == h64(0xFD)), "{err:?}");

        let err = apply_palw_transition_v2(&s2, &p, &point, &[derived_object(0xFC, 0x71, h64(0xBAD), fp_executor_key())], None);
        assert!(matches!(err, Err(PalwStateV2Error::DerivedOutputRootMismatch { .. })), "{err:?}");

        let mut stranger = fp_executor_key();
        stranger[0] ^= 1;
        let err = apply_palw_transition_v2(&s2, &p, &point, &[derived_object(0xFC, 0x71, output_root, stranger)], None);
        ...
        // Up to the cap, then the next is refused and nothing else about the state changes.
        ...
        let (a1, _) = apply(&genesis, &p, &ctx(1, 100, 1), &objects, None);
        let env = attempt(40, 1);
        let attempt_claim = attempt_id_v2(&env.attempt);
        let (a2, _) = apply(&a1, &p, &ctx(2, 101, 2), &[], Some(&env));
        let attempt_root = a2.claim(&attempt_claim).unwrap().output_root;
        let mut object = derived_object(0, 0x71, attempt_root, fp_executor_key());
        if let PalwConsensusObjectV2::DerivedArtifactV1 { object: o, .. } = &mut object {
            o.claim_id = attempt_claim;
        }
        let err = apply_palw_transition_v2(&a2, &p, &ctx(3, 102, 3), &[object], None);
        assert!(matches!(err, Err(PalwStateV2Error::DerivedClaimIsNotFreePrompt(_))), "{err:?}");

        // Shape: kind 0 is refused before any state is read.
        let mut object = derived_object(0xFC, 0x71, output_root, fp_executor_key());
        if let PalwConsensusObjectV2::DerivedArtifactV1 { object: o, .. } = &mut object {
            o.kind = 0;
        }
        let err = apply_palw_transition_v2(&s2, &p, &point, &[object], None);
        assert!(matches!(err, Err(PalwStateV2Error::DerivedShapeRefused(_))), "{err:?}");
    }

    /// **Decision 4: the rows retire with the claim (`CLAIM_RETIREMENT`).** A derivation is
    /// accepted in any phase from committed onward — here on a claim that then voids at its bind
    /// deadline — and the retirement sweep takes the rows with the claim; reverting the sweep
    /// puts them back byte for byte.
    #[test]
    fn a_claims_derivations_retire_with_it() {
        let p = params().with_claim_retirement_daa(50).unwrap();
        let s2 = fp_state_with_claim(&p);
        let output_root = s2.claim(&h64(0xFC)).unwrap().output_root;
        let (s3, _) = apply(
            ...
            &p,
            &ctx(3, 102, 3),
            &[derived_object(0xFC, 0x71, output_root, fp_executor_key()), derived_object(0xFC, 0x72, output_root, fp_executor_key())],
            None,
        );
        assert_eq!(s3.derived_artifacts_of(h64(0xFC)).count(), 2);
        ...
        // Still accepted after the void: "any phase from committed onward".
        let (s5, _) = apply(&s4, &p, &ctx(5, voided_at + 1, 5), &[derived_object(0xFC, 0x73, output_root, fp_executor_key())], None);
        assert_eq!(s5.derived_artifacts_of(h64(0xFC)).count(), 3);
        let (s6, retire) = apply(&s5, &p, &ctx(6, voided_at + 51, 6), &[], None);
        assert!(s6.claim(&h64(0xFC)).is_none(), "the claim retired");
        assert_eq!(s6.derived_artifacts_of(h64(0xFC)).count(), 0, "and its derivations with it");
        assert_eq!(s6.derived_artifacts_iter().count(), 0);
        let restored = revert_delta_v2(&s6, &retire, &p).expect("the retirement reverts");
        assert_eq!(restored.state_root(), s5.state_root());
        assert_eq!(restored.derived_artifacts_of(h64(0xFC)).count(), 3);
    }

    /// The carriage carries the table, the digest sees it, and a tampered row is refused by the
    /// root check like every other collection.
    #[test]
    fn derived_rows_ride_the_carriage_and_a_tampered_row_is_refused() {
        let p = params();
        let s2 = fp_state_with_claim(&p);
        let output_root = s2.claim(&h64(0xFC)).unwrap().output_root;
        ...
        assert_eq!(carriage.derived_artifacts.len(), 1);
        let reloaded = carriage.clone().into_state(&p, Some(s3.state_root())).expect("loads");
    ...
    /// `apply`, with ADR-0069 Decision 7 ARMED — the only position where a retiring free-prompt
    /// claim books its spent weight into `retired_safe_weight`.
    ///
    /// The 2026-09-02 audit's remedy was to book it unconditionally; the fold on this branch
    /// fences it, because `retired_safe_weight` is hashed into `state_root` and booking it below
    /// the fence forks the chain that is already running. Dormant is therefore the broken
    /// arithmetic ON PURPOSE, pinned as an executable fact by
    /// `known_limitation_dormant_a_spent_free_prompt_retirement_strands_its_weight` in
    /// `consensus/core/tests/palw_adr0069_d7_fold.rs`. A test asserting the repair has to arm it.
    ///
    /// Argument order is `unavailable_abstains, capability_bound, uncertified_weightless,
    /// da_court`; only the third is `true` here.
    fn apply_d7_armed(
        parent: &PalwChainStateV2,
        p: &PalwStateParamsV2,
        c: &PalwBlockContextV2,
    ...
    fn a_retired_free_prompt_claim_carries_the_weight_its_spends_added() {
        ...
        // `deadline < daa_score`). `apply_d7_armed` runs both consistency checks: against the
        // unfixed arm this line does not assert anything, it fails inside the helper. ARMED,
        // because the repair rides ADR-0069 Decision 7's fence on this branch — see the helper.
        let (retired, _) = apply_d7_armed(&s7, &p, &ctx(8, 400, 8), &[], None);
        ...
        let (after, _) = apply_d7_armed(&retired, &p, &ctx(9, 401, 9), &[], None);
    ...
    fn a_pruning_point_carriage_round_trip_completes_the_group() {
        ...
        // The completing chunk lands on BOTH and produces the same root.
        ...
        let group = pruned_done.court_close_group(&session_id, PalwCourtSideV1::Executor).unwrap();
        assert!(group.is_complete(), "the group is not complete after its last index arrived");
        assert_eq!(group.present, 0b111);
    ...
    fn adr_0043_spec_root(c: &PalwStateCarriageV2) -> Hash64 {
        ...
        spec_hash(b"misaka-palw/state-v2/state-root/v1", |s| {
            ...
            // ADR-0056 Decisions 3 and 5, in the position the implementation roots them.
            ...
            s.update(spec_collection_root(b"court_close_groups", &c.court_close_groups).as_byte_slice());
            s.update(spec_collection_root(b"derived_artifacts", &c.derived_artifacts).as_byte_slice());
    ...
    fn m02_populated_state() -> PalwChainStateV2 {
        ...
        state.derived_artifacts.insert(
            crate::palw_derived_v1::PalwDerivedKeyV1 { claim: h64(0xE2), transformer: h64(0xF1) },
            crate::palw_derived_v1::PalwDerivedRowV1 {
                derived_id: h64(0xF2),
                grammar_id: h64(0xF3),
                kind: crate::palw_derived_v1::kind::SCENE,
                dsl_hash: h64(0xF4),
                artifact_hash: h64(0xF5),
                artifact_bytes: 4_096,
                accepted_daa: 61,
            },
        );
        ...
        // so the inhabited root covers a `chunks`/`present` pair that is neither empty nor full.
        state.court_close_groups.insert(
            (h64(0xD1), PalwCourtSideV1::Executor),
            PalwCourtCloseGroupV2 {
                ...
                verdict: PalwCourtVerdictV2::ExecutorGuilty,
                declared_daa: 100,
                assembly_deadline_daa: 108,
                present: 0b01,
                deposit_outpoint: TransactionOutpoint::default(),
                deposit: 0,
                chunks: [(0u8, vec![7u8; 16])].into_iter().collect(),
            },
        );
    ...
    fn every_primary_datum_moves_the_root() {
        ...
        let perturbations: Vec<Perturb> = vec![
            ...
            (
                ...
                Box::new(|s| {
                    s.derived_artifacts
                        .get_mut(&crate::palw_derived_v1::PalwDerivedKeyV1 { claim: h64(0xE2), transformer: h64(0xF1) })
                        .unwrap()
                        .artifact_bytes += 1
                }),
            ),
            (
                "court_close_groups",
                Box::new(|s| s.court_close_groups.get_mut(&(h64(0xD1), PalwCourtSideV1::Executor)).unwrap().declared_daa += 1),
            ),
    ...
    fn da3_a_valid_disclosure_charges_the_accuser_and_nobody_else() {
        ...
        // The claim is back in the phase the accusation found it in, with the ONE DAA the session
        // took from it given back — not restarted (SA-7). It was bound at 102, accused at 103 and
        // answered at 104, so the receipt window it is serving now runs from 103.
        assert!(
            matches!(s5.claim(&claim_id).unwrap().phase, PalwClaimPhaseV2::PanelBound { bound_daa: 103 }),
            "the refuted claim resumes the phase it held, its anchor advanced by the session it spent answering"
        );
    ...
    // ---- ADR-0062 SA-7: what an accusation costs, what it buys, and what it may not suspend ----

    /// **SA-7(a): an open accusation does not close the ARITHMETIC court.**
    ///
    /// The two courts answer different questions, and a producer whose data is available can still
    /// be a producer whose arithmetic is fraudulent. While `validate_court_opened_v2` read
    /// `claim.phase` directly, one accusation — at `min(⌈reserved/seats⌉, min_collateral_sompi)`,
    /// from any bond including the producer's own second one — made the claim un-`ReceiptLicensed`
    /// and therefore un-challengeable for every remaining DAA of its challenge window, against a
    /// `CourtFraud` conviction worth `claim.reserved` and the whole escrow. The immunity was
    /// cheaper than the fraud by construction whenever `seat_count ≥ 2`.
    ///
    /// So the surface is read through the disputed phase, and the part of the window the session
    /// has eaten is added back — otherwise looking through it would only make the same purchase
    /// slower. The claim stays challengeable at every DAA of the session, and the DA session's own
    /// deadline survives a court opening: disarming it would make opening a court the way to cancel
    /// an accusation.
    #[test]
    fn an_open_accusation_does_not_close_the_arithmetic_court() {
        ...
        let challengeable = |state: &PalwChainStateV2, daa: u64| {
            crate::palw_court_v2::validate_court_opened_v2(
                state,
                &p,
                &ctx(9, daa, 9),
                &sid,
        ...
        let lapses_at = 103 + p.window_challenge();
        assert!(challengeable(&licensed, 104).is_ok(), "the licensed claim is in its challenge surface");
        assert!(challengeable(&licensed, lapses_at).is_ok(), "…up to and including the last DAA of the window");
        assert!(challengeable(&licensed, lapses_at + 1).is_err(), "…and not past it");

        // One accusation, at the first DAA of the challenge window.
        let (accused, _) = apply_da(&licensed, &p, &ctx(5, 104, 5), &[da_accuse(claim_id, 1)], None);
        let da_deadline = 104 + palw_da_disclose_window_daa_v1(&p);
        for daa in [105u64, 110, 120, lapses_at, lapses_at + 1, da_deadline] {
            assert!(
                challengeable(&accused, daa).is_ok(),
                "an accusation may not buy immunity from the fraud court at DAA {daa} — the claim is not Final and a claim \
                 that is not Final must be challengeable"
            );
        }

        // And a court that DOES open on a disputed claim leaves the silence clock alone.
        let (both, _) = apply_da(&accused, &p, &ctx(6, 106, 6), &[court_open(claim_id, trace_root, bond_key(1), bond_key(2))], None);
        assert!(
            both.deadlines.contains(&(da_deadline, claim_id)),
            "opening a court must not disarm the accusation's own deadline — that would make it the way to cancel one"
        );
        ...
        assert_eq!(
            refuted.claim(&claim_id).unwrap().phase,
            PalwClaimPhaseV2::ReceiptLicensed { licensed_daa: 103 },
            "a producer that has finished its own work is only waiting, so its wait is not extended"
        );
        assert_eq!(refuted.open_courts_by_claim.get(&claim_id).copied(), Some(1), "the arithmetic court is still open");
    }

    /// **SA-7(b): two accusations answered correctly and on time do not destroy an honest claim.**
    ///
    /// `W_disclose = window_challenge`, and every ConsensusV2 preset has `window_challenge` strictly
    /// greater than both `window_receipt` and `window_bind` (shipped reference bundle: 600 / 600 /
    /// 1200). So a claim accused while `PanelBound` and restored to the deadline it held BEFORE the
    /// session came back to a receipt window that had already lapsed — guaranteed, not by accident —
    /// and the very next sweep redrew it. The redraw leaves the old panel record in place, so a
    /// second accusation was admissible at once, and the second honest answer came back to an
    /// expired BIND deadline: `Voided { BindTimeout }`. Two accusations at the registry floor, and a
    /// producer that answered both correctly and on time lost its escrow. A responder cannot help,
    /// because ANSWERING is what triggers it.
    ///
    /// The fix is that the pause is a pause: the anchor advances by exactly the session's length, so
    /// the producer gets back the window it had left and not one DAA more.
    #[test]
    fn two_answered_accusations_do_not_destroy_an_honest_producers_claim() {
        let p = params();
        ...
        assert!(
            ...
            r1.claim(&claim_id).unwrap().phase
        );
        ...
        assert!(
            ...
            r2.claim(&claim_id).unwrap().phase
        );
        assert!(r2.claim(&claim_id).unwrap().rebound_daa.is_none(), "nothing was redrawn — no window ever lapsed");

        // The proof it costs the producer nothing but time: it licenses and finalizes normally.
        let licensed_at = at2 + window + 1;
        let (licensed, _) = apply_da(
            &r2,
            &p,
            &ctx(8, licensed_at, 8),
        ...
        let (finalized, _) = apply_da(&licensed, &p, &ctx(9, licensed_at + p.window_challenge() + 1, 9), &[], None);
        assert!(
            matches!(finalized.claim(&claim_id).unwrap().phase, PalwClaimPhaseV2::Final { .. }),
            "an honest producer that answered every accusation reaches Final: got {:?}",
            finalized.claim(&claim_id).unwrap().phase
        );
        assert_eq!(finalized.bond(&bond_key(1)).unwrap().slashed, 0, "and is never charged for having been accused");
        assert_eq!(
            finalized.bond(&bond_key(2)).unwrap().slashed,
            2 * palw_da_accusation_exposure_v2(200, 2, p.min_collateral_sompi()) as u64,
            "while the accuser paid for both of its wrong accusations"
        );
    }

    /// **SA-7(c): one bond's total data-availability liability is its exposure ceiling.**
    ///
    /// Being Active and above the floor is a check on the accuser's STATE, and reserving does not
    /// change it — `write_exposure` moves the exposure ledger and leaves `collateral` alone. So
    /// without a ceiling one bond holding exactly `min_collateral_sompi` passed the same two checks
    /// once per live claim on the network and froze all of them, in one block if it liked; and
    /// because `slash_bond` debits `min(amount, collateral)` and returns early at zero, the FIRST
    /// refutation emptied the bond and every later one cost nothing. K frozen claims for one
    /// registry floor.
    ///
    /// The ceiling the `CourtOpened` arm says "does the counting" is applied only in
    /// `check_palw_attempt_admission_v2` — i.e. only to a bond that wants to PRODUCE. Applied here
    /// too, and with the reservation capped at the figure `slash_seat` can actually take, the K-th
    /// accusation is funded exactly like the first.
    #[test]
    fn one_bond_at_the_floor_can_freeze_exactly_one_claim() {
        ...
        for (bond, pubkey, op, collateral) in
            [(bond_key(2), vec![8; 4], op_key(22), 1_000u64), (bond_key(3), vec![6; 4], op_key(23), floor)]
        {
            objects.push(PalwConsensusObjectV2::BondRegistered {
                bond,
                pubkey,
        ...
        // Two live claims, each with a two-seat panel — the network an attacker wants to freeze.
        let mut state = s1;
        ...
        // The first accusation exhausts the floor bond's whole ceiling.
        let (one, _) = apply_da(&state, &p, &ctx(30, 105, 6), &[accuse(claims[0])], None);
        assert_eq!(one.reserved_exposure(&bond_key(3)), floor as u128, "the accusation reserves the whole ceiling");
        assert_eq!(one.bond(&bond_key(3)).unwrap().collateral, floor, "and reserving is still not charging");
    ...
    ///
    /// `slash_seat` caps the charge at `min_collateral_sompi`, for the reason its own doc gives:
    /// both factors of `claim.reserved` are chosen by whoever registered the CLASS, so an uncapped
    /// charge would let a registrant make accusing its own class ruinous. The RESERVATION was the
    /// uncapped fraction, so for any claim with `reserved / seat_count > min_collateral_sompi` the
    /// ledger recorded a liability the fold could never collect — and the ceiling above then counted
    /// money that did not exist.
    #[test]
    fn an_accusation_reserves_exactly_what_the_fold_can_take() {
        let p = params();
        let floor = p.min_collateral_sompi();
        ...
        assert_eq!(palw_da_accusation_exposure_v2(reserved, 2, floor), floor as u128, "and the price is the floor");
