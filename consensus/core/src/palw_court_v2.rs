...
pub enum PalwCourtV2Error {
    ...
    /// **ADR-0080 W6.** Named apart from [`Self::RungSignatureInvalid`] because the two are acted
    /// on differently by whoever reads the log: a rung's bad signature is one refused move in a
    /// live ladder, and this one is a whole carriage — a declaration plus every chunk behind it —
    /// that will never open a group.
    #[error(
    ...
    /// **ADR-0080 W6, the ruleset's own count.** The transition enforces the structural bound the
    /// `u64` bitmap can address ([`crate::palw_state_v2::PALW_COURT_CLOSE_MAX_CHUNKS`]); this is
    /// the NETWORK's, which is inside `palw_ruleset_id_v2` and is 1 on devnet — where a legal close
    /// still has to fit one carrier and the split path must be refused rather than engaged.
    #[error(
...
mod tests {
    ...
    fn the_court_domains_are_distinct_from_nothing_by_accident() {
        // The party id, the two rung signing contexts, the open context, and — since ADR-0080 W6 —
        // the close declaration's. The count is pinned so ADDING a domain is a decision someone
        // makes here rather than a line that slips in; the cross-family collision test reads the
        // list itself.
        assert_eq!(PALW_COURT_V2_ALL_DOMAINS.len(), 5);
        assert!(
