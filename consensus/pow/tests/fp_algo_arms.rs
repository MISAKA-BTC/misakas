...
fn a_v2_lineage_header_without_its_carriage_is_a_failed_pow_not_a_panic() {
    ...
    for header in [&attempt_header, &spend_header, &crossed_attempt, &crossed_spend] {
        ...
        assert_eq!(
            ...
            PowLayer0Error::PalwCarriageMissing(header.pow_algo_id),
...
fn the_attempt_lane_prices_its_digest_and_binds_every_priced_field() {
    ...
    let mut impossible = solved.clone();
    ...
    // Every field the commitment root prices moves the digest, through the live path.
    let mut envelope = PalwAttemptEnvelopeV2::decode(&carriage).unwrap();
    let mutations: Vec<(&str, fn(&mut PalwAttemptUnsignedV2))> = vec![
    ...
    for (field, mutate) in mutations {
        ...
        let header = header_with(POW_ALGO_ID_PALW_COMMITTED_V2, mutated.encode());
        let state = kaspa_pow::StateLayer0::new(&header, NETWORK);
    ...
    let resigned = header_with(POW_ALGO_ID_PALW_COMMITTED_V2, envelope.encode());
...
fn the_receipt_lane_binds_identity_without_pricing_or_buying_level() {
    ...
    let mut impossible = header.clone();
    impossible.bits = 0x03000001;
    ...
    let mut attempt_at_same_bits = header_with(POW_ALGO_ID_PALW_COMMITTED_V2, attempt_carriage(&base));
