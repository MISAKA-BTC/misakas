//! The legacy PALW (`algo_id = 4` / `5`) PoW tag driver — every path by which a tag REACHES a
//! model runtime, extracted from `kaspa-pow` (ADR-0042 Decision 4, PR-02).
//!
...
//! canonical prompt, the pinned worker (or the host-local Ollama server) greedily decodes under
//! the frozen ceiling, and the replay-stable projection fields become the Layer-1 tag.
//! Verification IS re-execution — the Open-then-Audit small-`q` full-replay regime the V1 lineage
//! runs on, and exactly what ADR-0042 Decision 4 forbids the consensus build from containing.
//!
//! The boundary, and who stands on which side of it:
//!
//! * `kaspa-pow` keeps the pure half — seed/prompt derivation, the devnet fixture family, the
//!   calibration POLICY — and a set-once [`PalwPowRuntime`] slot. With nothing in the slot, an
//!   inference-priced tag is [`PowLayer0Error::PalwUnavailable`] and the header fails PoW: a full
//!   node without a model is the normal case, not a fault.
//! * This crate is the impure half: `palw-worker` subprocesses, the resident pow-agent pool, the
//!   host-local Ollama HTTP runner, per-process tag caches, the host-wide inference lease. A
//!   composition root that WANTS the legacy lane — kaspad on a network whose rules can demand
//!   algo 4/5, a miner producing such blocks — calls [`install`] at startup. Nothing else links
//!   this crate, and a CI test on the dependency graph (`no_model_runtime_edge.rs` in
//!   `kaspa-pow`) keeps it that way.
//!
//! Worker failures are never header-dependent: the prompt is a fixed ASCII frame around 64 hex
//! chars (comfortably under the ceiling), so a timeout / non-zero exit / unparseable document is
//! environmental and surfaces as [`PowLayer0Error::PalwWorkerFailed`] — which the consensus
//! wrapper still escalates loudly, because a node that HAS a runtime and watches it die
//! persistently must not fall into silently rejecting every valid header.

use kaspa_consensus_core::pow_layer0::{
    POW_L1_PALW_N_PREDICT_V1, POW_L1_PALW_OLLAMA_OUT_BYTES, POW_L1_PALW_OUT_BYTES, POW_L1_PALW_PROBE_SEED_V1, PowLayer0Error,
    palw_l1_tag_from_projection, palw_pow_prompt_v1,
};
use kaspa_hashes::Hash64;
use kaspa_pow::palw::{PALW_FIXTURE_ENV, PalwPowRuntime, inference_concurrency, register_palw_pow_runtime};
...
///
/// [`kaspa_pow::palw::PALW_CONCURRENCY_ENV`] alone bounds a process, and the resource it protects
/// is the host: two PALW nodes on one box are two concurrent inferences whatever either process's
/// semaphore says, and that configuration measured **0.38× the serial throughput** (ADR-0041
/// Decision 2) — entered by co-locating nodes, without anyone touching the knob. With this set, a
/// permit additionally requires an exclusive `flock` on one of `MISAKA_PALW_CONCURRENCY` slot
...
/// The one implementation of [`PalwPowRuntime`] in this tree. A unit struct on purpose: every
/// piece of state it manages — tag caches, the inference gate, the agent pool, memoized probes —
/// is process-scoped, exactly like the set-once slot it is registered into.
pub struct PalwPowDriver;

static DRIVER: PalwPowDriver = PalwPowDriver;

/// Register this driver as the process's PALW PoW runtime.
///
/// Called once by a composition root that wants the legacy lane — kaspad's startup rail when the
/// network's rules can ever demand algo 4/5, a miner before it grinds such a template, a driver
/// test binary. The slot is set-once per process and this is the only in-tree implementation, so
/// a second call finds the work already done and is a no-op rather than an error: two paths of
/// one composition root agreeing that the legacy lane exists is agreement, not a conflict.
pub fn install() {
    let _ = register_palw_pow_runtime(&DRIVER);
}

impl PalwPowRuntime for PalwPowDriver {
...
pub fn verify_ollama_model_pin(url: &str, model: &str) -> Result<(), PowLayer0Error> {
    verify_model_pin(url, model)?;
    verify_calibration(url, model)
}
...
pub fn palw_generate(
    ...
    generate(url, model, prompt, num_predict, templated, think)
}
...
/// matters here: the consensus wrapper panics on a runtime that persistently fails, and a permit
/// leaked on that route would wedge every other validating thread behind a node that is already
/// failing loudly.
struct Permit {
...
fn tag_for_seed(seed: &[u8; 32]) -> Result<[u8; POW_L1_PALW_OUT_BYTES], PowLayer0Error> {
...
/// kill, or a timeout under validation load) as well as a permanent one, and the consensus
/// wrapper (`calc_block_level_check_pow_layer0`) panics on what survives these attempts.
/// Panicking a node because one subprocess lost a race under load is a self-inflicted outage;
/// three attempts convert the common transient into a delay, and a genuinely broken runtime
...
fn timeout() -> Duration {
    Duration::from_secs(std::env::var(PALW_TIMEOUT_ENV).ok().and_then(|s| s.parse().ok()).unwrap_or(DEFAULT_TIMEOUT_SECS))
}
...
///   ships today;
/// * the document it returns is read by [`tag_from_doc`], the same code that reads a
...
fn ollama_tag_for_seed(seed: &[u8; 32]) -> Result<[u8; POW_L1_PALW_OLLAMA_OUT_BYTES], PowLayer0Error> {
...
/// network that pins a class — and it is the same shape as the confinement bug the fixture
/// family once had: a process-global answer to a per-network question.
static WORKER_CALIBRATION_PROBE: OnceLock<Result<String, String>> = OnceLock::new();

fn worker_calibration_once(expected: &'static str) -> Result<(), PowLayer0Error> {
...
fn verify_calibration(url: &str, model: &str) -> Result<(), PowLayer0Error> {
...
fn verify_model_pin(url: &str, model: &str) -> Result<(), PowLayer0Error> {
...
/// this driver. `body = None` sends a bodyless request (GET).
fn http_request(method: &str, base_url: &str, path: &str, body: Option<&str>, budget: Duration) -> Result<Vec<u8>, PowLayer0Error> {
    ...
    let mut stream = TcpStream::connect(hostport).map_err(|e| {
        PowLayer0Error::PalwUnavailable(format!("cannot reach the Ollama server at {hostport}: {e} (is `ollama serve` running?)"))
    })?;
    ...
    stream
        .read_to_end(&mut raw)
        .map_err(|e| PowLayer0Error::PalwWorkerFailed(format!("reading the Ollama response failed (budget {budget:?}): {e}")))?;
    let header_end = raw
        .windows(4)
        .position(|w| w == b"\r\n\r\n")
...
fn parse_projection(stdout: &[u8]) -> Result<[u8; POW_L1_PALW_OUT_BYTES], PowLayer0Error> {
    ...
    let doc: serde_json::Value =
        serde_json::from_str(line).map_err(|e| PowLayer0Error::PalwWorkerFailed(format!("cannot parse the worker document: {e}")))?;
    tag_from_doc(&doc)
}
